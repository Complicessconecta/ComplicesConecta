...
export const useCouplePhotos = (profileId?: string): UseCouplePhotosReturn => {
  ...
  const loadPhotos = async () => {
    ...
    try {
      ...
      const { data, error: fetchError } = await supabase
        ...
        .select("avatar_url, id, created_at")
        .eq("id", currentProfileId!)
        .single();

      if (fetchError) throw fetchError;

      const photosWithUrls = data?.avatar_url
        ? [{
            id: `${data.id}-0`,
            url: data.avatar_url,
            partner: "el" as "el" | "ella",
            isMain: true,
            profileId: data.id,
      ...
      logger.error("Error loading couple photos:", {
  ...
  const uploadPhoto = async (file: File, partner: "el" | "ella") => {
    ...
    try {
      ...
      // Actualizar el perfil con la nueva imagen como avatar si es la primera foto
      if (isFirstPhoto) {
      ...
      const newPhotoData: CouplePhoto = {
        id: `${currentProfileId}-${photos.length}`,
        url: publicUrl,
        partner,
        isMain: isFirstPhoto,
        uploadedAt: new Date(),
  ...
  const deletePhoto = async (photoId: string) => {
    try {
      ...
      if (supabase.storage) {
        ...
        if (storageError) {
          logger.warn("Error deleting from storage:", {
            error: storageError.message,
          });
        }
      }

      // Eliminar de la base de datos (si es la foto principal, establecer avatar_url a null)
    ...
    try {
      ...
      setPhotos((prev) =>
        prev.map((photo) => ({
          ...photo,
          isMain:
            photo.partner === partner ? photo.id === photoId : photo.isMain,
        })),
      );
    } catch (err) {
      logger.error("Error setting main photo:", {
        error: err instanceof Error ? err.message : String(err),
      });
