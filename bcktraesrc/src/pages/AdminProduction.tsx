...
const AdminProduction = () => {
  ...
  useEffect(() => {
    ...
    // CR├ìTICO: No verificar autenticaci├│n si a├║n est├í cargando
    if (loading) {
      logger.info('ÔÅ│ useAuth a├║n cargando - esperando...');
      return;
    }
    ...
    if (demoAuth === 'true' && demoUser) {
      try {
        ...
        if (user.accountType === 'admin' || user.role === 'admin') {
          logger.info('Ô£à Admin demo autorizado - cargando panel producci├│n');
          loadProductionData();
          return;
        } else {
    ...
    logger.info('­ƒöÉ Estado autenticaci├│n:', { status: authStatus });
    ...
    logger.info('Ô£à Acceso autorizado - cargando panel producci├│n');
  ...
  const loadRealStats = async () => {
    try {
      ...
      // Obtener estad├¡sticas b├ísicas de profiles
      const [
        { count: totalUsers },
        { count: premiumUsers },
        { count: activeUsers },
      ...
      logger.info('­ƒôè Estad├¡sticas cargadas:', {
  ...
  const loadRealFAQ = async () => {
    ...
    logger.info('ÔÜá´©Å Tabla faq_items no disponible, usando lista vac├¡a');
    setFaqItems([]);
  };
  ...
  const handleAddFAQ = async () => {
    ...
    toast({
      ...
      description: "La tabla FAQ no est├í disponible en el esquema actual",
      variant: "destructive"
    });
  };
