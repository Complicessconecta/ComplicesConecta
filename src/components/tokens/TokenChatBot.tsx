...
¿Cuántos CMPX quieres meter en staking?`;
  ...
  const handleGreetingResponse = (response: string) => {
    ...
    if (
      ...
      addBotMessage(
        '😊 ¡Perfecto! Cuando quieras revisar tu saldo, nomás pregúntame.\n\n💡 También puedes decir:\n• "¿Cuántos tokens tengo?"\n• "Quiero hacer staking"\n• "¿Qué recompensas hay?"',
      );
    }
  };
      ...
      if (hasPendingRewards || isWorldIdEligible) {
        addBotMessage(getRewardsMessage(), [
          ...
          },
          {
            ...
            action: () =>
              addBotMessage(
                "😊 ¡Perfecto! Tus recompensas estarán aquí cuando las quieras reclamar.",
              ),
          },
        ]);
      } else {
...
¿Quieres revisar opciones de staking ahorita?`,
          [
            ...
            },
            {
              ...
              action: () =>
                addBotMessage(
                  "¡Perfecto! Tus tokens están seguros en tu saldo. ¡Que disfrutes ComplicesConecta! 🚀",
                ),
            },
          ],
        );
        ...
        addBotMessage(
          "⚠️ No se pudieron reclamar las recompensas. Inténtalo más tarde.",
        );
      }
    } catch {
      addBotMessage(
        "❌ Error procesando recompensas. Por favor inténtalo de nuevo.",
      );
    }
  };
  ...
  const handleStakingInput = async (amount: string) => {
    ...
    if (isNaN(stakingAmount) || stakingAmount < 50) {
      addBotMessage(
        "⚠️ Por favor ingresa un número válido. Mínimo 50 CMPX para hacer staking.",
      );
      return;
    }
    ...
    if (stakingAmount > (balance?.cmpxBalance || 0)) {
      addBotMessage(
        `⚠️ No tienes suficientes CMPX. Tienes ${balance?.cmpxBalance || 0} disponibles.`,
      );
      return;
    }
...
¿Confirmas que quieres iniciar el staking?`,
      [
        ...
        },
        {
          ...
          action: () =>
            addBotMessage(
              "😊 Staking cancelado. Tus CMPX siguen disponibles en tu saldo.",
            ),
        },
      ],
    );
...
¡Gracias por apoyar la red ComplicesConecta! 🌟`);
        ...
        addBotMessage(
          `❌ Error iniciando staking. Inténtalo de nuevo más tarde.`,
        );
      }
    } catch {
      addBotMessage(
        "❌ Error procesando staking. Por favor inténtalo de nuevo.",
      );
    }
  };
    ...
    switch (currentStep) {
      ...
      case "staking":
        if (/^\d+$/.test(input)) {
          ...
          addBotMessage(
            "😊 ¡Perfecto! El staking es opcional. Tus CMPX están seguros en tu saldo.\n\n¿Hay algo más en lo que te pueda ayudar?",
          );
        } else {
          addUserMessage(input);
          addBotMessage(
            '💡 Para hacer staking, ingresa la cantidad de CMPX (ejemplo: 100) o escribe "no" si prefieres no hacerlo ahorita.',
          );
        }
        break;
      ...
      default:
        ...
        if (
          ...
          lowerInput.includes("recompensa") ||
          lowerInput.includes("reclamar")
        ) {
          addBotMessage(getRewardsMessage());
        } else if (
          lowerInput.includes("staking") ||
          lowerInput.includes("alcancía")
        ) {
          addBotMessage(getStakingMessage());
        } else {
          addBotMessage(
