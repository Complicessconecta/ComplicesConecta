...
const Matches = () => {
  ...
  const [filter, setFilter] = useState<"all" | "new" | "recent" | "unread">(
    "all",
  );
  ...
  useEffect(() => {
    ...
    if (matches.length === 0) {
      ...
      logger.info("?? Matches demo cargados (respetando lgica de negocio):", {
  ...
  const filteredMatches = currentMatches.filter((match) => {
    switch (filter) {
      ...
      case "recent":
        return (
          match.matchedAt.includes("horas") || match.matchedAt.includes("Ayer")
        );
      case "unread":
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {[
              ...
              },
              {
                ...
                label: "Recientes",
                icon: Sparkles,
                count: currentMatches.filter(
                  (m) =>
                    m.matchedAt.includes("horas") ||
                    m.matchedAt.includes("Ayer"),
                ).length,
              },
              {
                id: "unread",
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="bg-gray-800/30 rounded-2xl p-4 animate-pulse h-80"
              >
          ...
          <div className="text-center py-16 bg-gray-800/30 rounded-2xl border border-dashed border-gray-700/50">
            ...
            <p className="text-gray-400 max-w-md mx-auto mb-6">
              No se encontraron coincidencias con los filtros actuales. Intenta
      ...
      <div className="p-4">
        ...
        <div className="mt-3 text-center">
          <span className="text-xs text-gray-400">
            Conectado {match.matchedAt.toLowerCase()}
          </span>
        </div>
      </div>
    </motion.div>
