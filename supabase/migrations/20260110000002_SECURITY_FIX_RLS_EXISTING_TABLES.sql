...
CREATE POLICY "Users can view accessible couple profiles" ON public.couple_profiles
    ...
    USING (
        user_id = auth.uid() OR
        partner_1_id = auth.uid() OR
        partner_2_id = auth.uid() OR
        EXISTS (
            SELECT 1 FROM public.admin_users 
            WHERE user_id = auth.uid() AND is_active = TRUE
        )
...
CREATE POLICY "Users can view own NFTs" ON public.user_nfts
    FOR SELECT
    USING (
        owner_address IN (SELECT address FROM public.user_wallets WHERE user_id = auth.uid()) OR
        EXISTS (
            SELECT 1 FROM public.admin_users
            WHERE user_id = auth.uid() AND is_active = TRUE
        )
...
CREATE POLICY "Users can insert own NFTs" ON public.user_nfts
    FOR INSERT
    WITH CHECK (owner_address IN (SELECT address FROM public.user_wallets WHERE user_id = auth.uid()));
...
CREATE POLICY "Users can update own NFTs" ON public.user_nfts
    FOR UPDATE
    USING (owner_address IN (SELECT address FROM public.user_wallets WHERE user_id = auth.uid()));
...
DROP POLICY IF EXISTS "Users can view own gallery permissions" ON public.gallery_permissions;
CREATE POLICY "Users can view own gallery permissions" ON public.gallery_permissions
    FOR SELECT
    USING (
        gallery_owner_id = auth.uid() OR
...
DROP POLICY IF EXISTS "Users can insert gallery permissions" ON public.gallery_permissions;
CREATE POLICY "Users can insert gallery permissions" ON public.gallery_permissions
    FOR INSERT
    WITH CHECK (gallery_owner_id = auth.uid());

DROP POLICY IF EXISTS "Owners can update gallery permissions" ON public.gallery_permissions;
