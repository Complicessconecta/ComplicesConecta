...
export class DataPrivacyService {
  ...
  async exportUserData(userId: string): Promise<UserDataExport | null> {
    try {
      logger.info("📥 Exportando datos del usuario", {
  ...
  async deleteUserAccount(
    ...
    try {
      logger.warn("🗑️ Iniciando eliminación de cuenta", {
        userId: userId.substring(0, 8) + "***",
      });
        ...
        if (images && images.length > 0) {
          ...
          for (const bucket of buckets) {
            try {
              ...
              logger.warn("Error eliminando imágenes de storage:", {
                error:
          ...
          if (deleteImagesError) {
            errors.push(
              `Error eliminando imágenes: ${deleteImagesError.message}`,
            );
          } else {
      ...
      try {
        ...
        if (messages && messages.length > 0) {
          ...
          if (anonymizeError) {
            errors.push(
              `Error anonimizando mensajes: ${anonymizeError.message}`,
            );
          } else {
      ...
      try {
        ...
        if (matches && matches.length > 0) {
          ...
          if (deleteMatchesError) {
            errors.push(
              `Error eliminando matches: ${deleteMatchesError.message}`,
            );
          } else {
      ...
      try {
        ...
        if (stories && stories.length > 0) {
          ...
          if (deleteStoriesError) {
            errors.push(
              `Error eliminando stories: ${deleteStoriesError.message}`,
            );
          } else {
      ...
      try {
        ...
        if (!deleteNotificationsError) {
          ...
          errors.push(
            `Error eliminando notificaciones: ${deleteNotificationsError.message}`,
          );
        }
      } catch (error) {
      ...
      logger.warn(
        "⚠️ Eliminación de auth.users requiere service role key - debe hacerse desde backend",
        {
          userId: userId.substring(0, 8) + "***",
        },
      );
      ...
      logger.info(
        success
          ...
          : "⚠️ Eliminación de cuenta completada con errores",
        {
          userId: userId.substring(0, 8) + "***",
          deletedItems,
  ...
  async deleteMatchHistory(
    ...
    try {
      logger.info("🗑️ Eliminando historial de matches", {
        userId: userId.substring(0, 8) + "***",
      });
  ...
  downloadExport(exportData: UserDataExport, filename?: string): void {
    try {
      ...
      link.download =
        filename ||
        `complicesconecta-data-export-${exportData.metadata.exportDate.split("T")[0]}.json`;
      ...
      logger.error("❌ Error descargando exportación:", {
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
    }
