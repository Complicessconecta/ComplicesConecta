...
export interface CreateReportParams {
  ...
  contentType: 'profile' | 'chat' | 'image' | 'post' | 'story' | 'message' | 'comment';
  reason: string;
  description?: string;
...
class ReportService {
   ...
   */
  async getPendingProfileReports(limit: number = 50): Promise<{ success: boolean; reports?: Report[]; error?: string }> {
    try {
   ...
   * Resolver un reporte
   */
  async resolveProfileReport(reportId: string, resolution: string): Promise<{ success: boolean; error?: string }> {
    try {
      if (!supabase) return { success: false, error: 'Supabase no disponible' };
   ...
   */
  async getPendingReports(): Promise<{ success: boolean; reports?: Report[]; error?: string }> {
    try {
      // Mock data
      ...
      logger.error('Error getting pending reports:', { error: error instanceof Error ? error.message : String(error) });
      return { success: false, error: 'Error al obtener reportes' };
    }
  }
   ...
   * Resolver un reporte
   */
  async resolveReport(reportId: string, action: string, notes: string): Promise<{ success: boolean; error?: string }> {
    try {
   ...
   */
  async getReportStatistics(): Promise<{ success: boolean; stats?: ReportStats }> {
    // Mock stats
    return {
      success: true,
      stats: {
