...
class ConsentVerificationService {
  ...
  async analyzeConsent(
    ...
    try {
      ...
      // 3. Combinar análisis
      const consentLevel = this.determineConsentLevel(
        patternAnalysis,
        contextAnalysis,
      );
      ...
      logger.error("Error analizando consentimiento:", {
        error: String(error),
      });
  ...
  async verifyConsentBeforeSend(
    ...
    try {
      ...
      if (
        ...
        analysis.consentLevel !== "explicit"
      ) {
      ...
      logger.error("Error verificando consentimiento:", {
        error: String(error),
      });
      throw error;
    }
  ...
  private async analyzeContext(
    ...
    try {
      ...
      if (metadata?.previousMessages) {
        ...
        const hasUrgentKeywords = /(urgente|ahora|inmediato|rápido)/gi.test(
          message,
        );
        const hasPositiveKeywords = /(bueno|genial|excelente|perfecto)/gi.test(
          message,
        );
        ...
        return {
          ...
          requiresExplicitConsent: this.REQUIRES_EXPLICIT_CONSENT.includes(
            metadata?.messageType || "",
          ),
        };
      }
  ...
  private determineConsentLevel(
    ...
    if (explicit === 0 && negative === 0 && ambiguous === 0) {
      return contextAnalysis.sentiment === "positive"
        ? "implicit"
        : "ambiguous";
    }
  ...
  private calculateConfidence(
    ...
    const totalPatterns =
      patternAnalysis.explicit +
      patternAnalysis.negative +
      patternAnalysis.ambiguous;
  ...
  private generateExplanation(
    ...
    return (
      explanations[consentLevel] ||
      "Análisis no concluyente. Se requiere revisión manual."
    );
  }
  ...
  async saveVerification(
    ...
    try {
      if (!supabase) {
        logger.error(
          "Supabase no está disponible para guardar verificación de consentimiento",
        );
        throw new Error(
          "No se pudo registrar la verificación de consentimiento",
        );
      }
      ...
      if (error) {
        logger.error(
          "No se pudo guardar verificación de consentimiento en consent_verifications. Crear migración / revisar BD.",
          {
            error: error.message,
          },
        );
  ...
  async getVerificationHistory(
    ...
    try {
      ...
      const minimalClient =
        supabase as unknown as MinimalSupabaseClientForHistory;
      ...
      return rows.map((item) => {
        ...
        const consent_level = item.consent_level as
          | ConsentAnalysis["consentLevel"]
          | undefined;
        const confidence = item.confidence as number | undefined;
        const requires_confirmation = item.requires_confirmation as
          | boolean
          | undefined;
        const suggested_action = item.suggested_action as
          | ConsentAnalysis["suggestedAction"]
          | undefined;
        ...
        return {
          ...
          verified:
            typeof verified_flag === "boolean"
              ? verified_flag
              : Boolean(verified_at_raw),
