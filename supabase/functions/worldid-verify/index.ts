...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Parse request body
    const { proof, user_id, invited_by }: VerificationRequest =
      await req.json();
    ...
    const worldcoinResponse = await fetch(
      Deno.env.get("WORLD_VERIFY_ENDPOINT") ??
    ...
    const worldcoinData: WorldcoinVerifyResponse =
      await worldcoinResponse.json();
    ...
    const { data: rewardResult, error: rewardError } = await supabaseClient.rpc(
      "process_worldid_verification_reward",
      {
        p_user_id: user_id,
    ...
    if (!rewardResult.success) {
      return new Response(JSON.stringify(rewardResult), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
