...
// Mock speakeasy
vi.mock('speakeasy', () => ({
  generateSecret: vi.fn().mockReturnValue({ base32: 'SECRET', otpauth_url: 'otpauth://...' }),
  totp: {
    verify: vi.fn(),
  },
}));
...
describe('SecurityService', () => {
  ...
  describe('analyzeUserActivity', () => {
    ...
    it('should detect unusual activity based on history', async () => {
        ...
        const manyEvents = Array(100).fill({ event_type: 'action', created_at: new Date().toISOString() });
  ...
  describe('verify2FA', () => {
    ...
    it('should accept backup code', async () => {
        (supabase.from as any).mockReturnValue({
            ...
            eq: vi.fn().mockReturnThis(),
            single: vi.fn().mockResolvedValue({ 
                data: { secret: 'SECRET', is_enabled: true, backup_codes: ['BACKUP1'] }, 
                error: null 
            }),
