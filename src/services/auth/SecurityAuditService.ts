...
export class SecurityAuditService {
  ...
  private async checkSuspiciousActivity(): Promise<void> {
    try {
      ...
      (recentEvents as unknown as SecurityEventRow[] | null)?.forEach(
        (event) => {
      ...
      logger.error("Error in checkSuspiciousActivity:", {
        error: String(error),
      });
    }
  }
  ...
  private async checkAnomalousAccess(): Promise<void> {
    try {
      ...
      if (error) {
        logger.error("Error checking anomalous access:", {
          error: error.message,
        });
        return;
      }
      ...
      (accessEvents as unknown as SecurityEventRow[] | null)?.forEach(
        (event) => {
  ...
  private async checkDataIntegrity(): Promise<void> {
    try {
      ...
      if (error) {
        logger.error("Error checking data integrity:", {
          error: error.message,
        });
        return;
      }
      ...
      (profiles as Array<{ id: string; name: string | null }> | null)?.forEach(
        (profile) => {
      ...
      const duplicateCount = Array.from(nameCounts.values()).filter(
        (count) => count > 1,
      ).length;
  ...
  private async checkSecurityConfiguration(): Promise<void> {
    try {
      ...
      if (error) {
        logger.error("Error checking security configuration:", {
          error: error.message,
        });
        return;
      }
      ...
      logger.error("Error in checkSecurityConfiguration:", {
        error: String(error),
      });
    }
  }
  ...
  async analyzeThreats(): Promise<ThreatDetection[]> {
    try {
      ...
      if (
        criticalEvents &&
        (criticalEvents as unknown as SecurityEventRow[]).length > 0
      ) {
        threats.push({
          ...
          affectedUsers: [
            ...new Set(
              (criticalEvents as unknown as SecurityEventRow[]).map(
                (e) => e.user_id || "",
              ),
            ),
          ] as string[],
  ...
  async generateSecurityReport(): Promise<SecurityReport> {
    try {
      ...
      const mappedEvents: SecurityEvent[] = (
        (recentEvents as unknown as SecurityEventRow[] | null) || []
      ).map((event) => ({
        id: event.id || "",
      ...
      logger.error("Error in generateSecurityReport:", {
        error: String(error),
      });
      throw error;
    }
  ...
  private async checkComplianceStatus(): Promise<
    SecurityReport["complianceStatus"]
  > {
