...
export const NFTWalletView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const generateNFTs = () => {
      ...
      const newNfts: NFT[] = [];
      const shuffledImages = [...MOCK_NFT_IMAGES].sort(
        () => 0.5 - Math.random(),
      );
    ...
    <Card className="bg-white/10 backdrop-blur-md border-white/20 shadow-xl overflow-hidden">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-white">
          ...
          <Badge
            variant="outline"
            className="ml-auto border-purple-400/50 text-purple-300"
          >
            {nfts.length} / 4
          </Badge>
        </CardTitle>
      ...
      <CardContent>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {nfts.map((nft) => (
            <div
              key={nft.id}
              className="group relative bg-black/40 rounded-xl overflow-hidden border border-white/10 hover:border-purple-500/50 transition-all duration-300"
            >
              <div className="aspect-square relative">
                ...
                <div className="absolute top-2 right-2">
                  <Badge
                    className={`${RARITY_COLORS[nft.rarity]} text-white text-xs border-0 shadow-lg`}
                  >
                    {nft.rarity}
                  </Badge>
                </div>
              ...
              <div className="p-3">
                ...
                <Button
                  ...
                  onClick={() =>
                    toast({
                      title: `Detalles de ${nft.name}`,
                      description: `ID: ${nft.id}\nRareza: ${nft.rarity}`,
                    })
                  }
                >
