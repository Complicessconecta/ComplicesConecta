...
export const StakingWidget: React.FC<StakingWidgetProps> = ({
  ...
  const [actionStatus, setActionStatus] = useState<
    "idle" | "staking" | "unstaking" | "success" | "error"
  >("idle");
    ...
    try {
      if (isDemoMode) {
        ...
        const result = await walletService.executeDemoAction(
          userId,
          "stake_nft",
          { amount: parseFloat(stakeAmount), type },
        );
      ...
      const errorMsg =
        error instanceof Error ? error.message : "Error desconocido al stakear";
    ...
    try {
      ...
      const errorMsg =
        error instanceof Error
          ? error.message
          : "Error desconocido al hacer unstake";
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div>
                      ...
                      <div className="font-medium">
                        {calculateAPY(position.vestingPeriodDays)}%
                      </div>
                    </div>
                    <div>
