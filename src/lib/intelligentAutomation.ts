...
export class IntelligentAutomationService {
  ...
  static async initialize(): Promise<void> {
    ...
    try {
      ...
      logger.error("❌ Error initializing automation service:", {
  ...
  private static async initializePerformanceMonitoring(): Promise<void> {
    try {
      ...
      logger.info(
        "📊 Performance monitoring initialized for automation service",
      );
    } catch (error) {
      logger.error("❌ Error initializing performance monitoring:", {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
  ...
  private static async loadRules(): Promise<void> {
    try {
      ...
      logger.error("Error loading automation rules:", {
        error: error instanceof Error ? error.message : String(error),
      });
      this.rules = [];
    }
    ...
    >[] = [
      {
        ...
        description:
          "Envía notificación de bienvenida cuando un usuario se registra",
        trigger: {
          type: "user_signup",
          config: {},
        },
  ...
  private static async setupTriggerListeners(): Promise<void> {
    try {
      if (!supabase) {
        logger.error(
          "Supabase no está disponible, no se pueden configurar listeners",
        );
        return;
      }
      ...
      logger.error("Error setting up trigger listeners:", {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
   ...
   */
  private static async handleTrigger(
    triggerType: string,
    ...
    try {
      ...
      logger.error("Error handling automation trigger:", {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
  ...
  private static async executeActions(
    ...
    for (const action of actions) {
      try {
        ...
        logger.error(
          `Error executing action ${action.type} for rule ${ruleId}:`,
          { error: error instanceof Error ? error.message : String(error) },
        );
  ...
  private static async executeSendNotification(
    ...
    // Solo agregar actionUrl si existe
    if (typeof config.action_url === "string" && config.action_url) {
      notificationData.actionUrl = config.action_url;
    }
   ...
   */
  private static async executeCreateMatchSuggestion(
    config: Record<string, string | number | boolean | null>,
    triggerData: Record<string, unknown>,
  ): Promise<void> {
    const userId = triggerData.user_id || triggerData.id;
    ...
    try {
      ...
      if (userProfile) {
        ...
        logger.info("Match suggestions created:", {
          userId,
          suggestions: potentialMatches?.length,
        });
      }
    } catch (error) {
      logger.error("Error creating match suggestions:", {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
   ...
   */
  private static async executeModerateContent(
    config: Record<string, string | number | boolean | null>,
    triggerData: Record<string, unknown>,
  ): Promise<void> {
    ...
    try {
      ...
      logger.error("Error in moderate content action:", {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
   ...
   */
  private static async executeAssignModerator(
    config: Record<string, string | number | boolean | null>,
    triggerData: Record<string, unknown>,
  ): Promise<void> {
    try {
      ...
      logger.error("Error assigning moderator:", {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
  ...
  private static async logExecution(
    ...
    try {
      ...
      logger.error("Error processing automation action:", {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
  ...
  static async updateRule(
    ...
    this.rules[ruleIndex] = {
      ...existingRule,
      ...updates,
      id: existingRule.id, // Asegurar que id siempre está presente
      updated_at: new Date().toISOString(),
    };

    logger.info("Automation rule updated:", this.rules[ruleIndex]);

    return this.rules[ruleIndex] || null;
  }
  ...
  static async triggerRule(
    ...
    try {
      ...
      logger.error("Error triggering rule manually:", {
        error: error instanceof Error ? error.message : String(error),
      });
      await this.logExecution(
        rule.id,
...
IntelligentAutomationService.initialize().catch((error) => {
  logger.error("Failed to initialize Intelligent Automation Service:", {
    error: error instanceof Error ? error.message : String(error),
  });
});
