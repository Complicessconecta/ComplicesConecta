...
const ProtectedMedia = ({
  ...
  React.createElement(
    "div",
    { "data-testid": "protected-media-element" },
          ...
          "button",
          {
            onClick: () =>
              (document.body.innerHTML +=
                "<div>La descarga no está permitida</div>"),
          },
          "Descargar",
        ),
        React.createElement(
          "button",
          {
            onClick: () =>
              (document.body.innerHTML +=
                "<div>El contenido no puede ser compartido</div>"),
          },
          "Compartir",
        ),
      ),
...
describe("Media Access Security", () => {
  ...
  describe("requestSecureMediaUrl", () => {
    ...
    it("should handle access denied errors", async () => {
      ...
      await expect(requestSecureMediaUrl("test-media-id")).rejects.toThrow(
        "Access denied",
      );
    });
  ...
  describe("uploadSecureMedia", () => {
    ...
    it("should validate file types", async () => {
      ...
      await expect(uploadSecureMedia({ file: mockFile })).rejects.toThrow(
...
describe("ProtectedMedia Component", () => {
  it("should render loading state initially", () => {
    ...
    expect(
      screen.getByText("Cargando contenido seguro..."),
    ).toBeInTheDocument();
  });

  it("should prevent right-click context menu", () => {
    const { container } = render(
      React.createElement(MediaViewer, { _mediaId: "test-media" }),
    );
  ...
  it("should show access denied message when unauthorized", async () => {
    ...
    render(
      React.createElement(ProtectedMedia, {
        _mediaId: "test-media-1",
...
describe("MediaViewer Component", () => {
  ...
  it("should handle share attempt blocking", () => {
    ...
    // Should show error toast (mocked)
    expect(
      screen.getByText(/contenido no puede ser compartido/),
    ).toBeInTheDocument();
  });
...
describe("MediaUploader Component", () => {
  ...
  it("should validate file types on selection", () => {
    ...
    render(
      React.createElement(MediaUploader, {
        _onUploadComplete: mockOnUploadComplete,
      }),
    );
  ...
  it("should show upload progress", async () => {
    ...
    try {
      ...
      if (elapsed >= maxTime) {
        console.warn(
          "⚠️ [MediaUploader Test] Timeout alcanzado, saliendo del test",
        );
