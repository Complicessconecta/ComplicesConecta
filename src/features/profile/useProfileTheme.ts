...
export const useDemoThemeConfig = () => {
  const [demoTheme, setDemoTheme] = usePersistedState<Theme>(
    "demo_theme",
    "dark",
  );
  const [navbarStyle, setNavbarStyle] = usePersistedState<NavbarStyle>(
    "demo_navbar_style",
    "solid",
  );
...
export const useProductionThemeConfig = () => {
  ...
  const [fallbackTheme, setFallbackTheme] = usePersistedState<Theme>(
    "user_theme",
    "dark",
  );
  const [fallbackNavbar, setFallbackNavbar] = usePersistedState<NavbarStyle>(
    "user_navbar_style",
    "solid",
  );
