...
export class Neo4jService {
  ...
  reinitialize(): void {
    ...
    const enabledProcess =
      process.env.VITE_NEO4J_ENABLED || process.env.NEO4J_ENABLED;
  ...
  async createUser(
    ...
    try {
      ...
      if (metadata.metadata) {
        ...
        if (metadata.metadata.age !== undefined)
          flatMetadata.age = metadata.metadata.age;
        if (metadata.metadata.location)
          flatMetadata.location = metadata.metadata.location;
        if (metadata.metadata.gender)
          flatMetadata.gender = metadata.metadata.gender;
      }
  ...
  async createMatch(
    user1Id: string,
    user2Id: string,
    metadata: RelationshipProperties = {},
  ): Promise<void> {
    if (!this.isEnabled || !this.driver) {
      logger.warn("⚠️ Neo4j deshabilitado. No se creó match:", {
        user1Id,
        user2Id,
      });
      return;
  ...
  async getMutualFriends(user1Id: string, user2Id: string): Promise<string[]> {
    ...
    try {
      ...
      const mutualFriends = result.records.map(
        (record) => record.get("friendId") as string,
      );
