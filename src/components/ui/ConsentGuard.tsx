...
export const ConsentGuard: React.FC<ConsentGuardProps> = ({
  ...
  const handleAccept = async () => {
    ...
    if (!supabase) {
      toast({
        title: "Error",
        description:
          "Supabase no está disponible. Revisa configuración o modo demo.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Error",
        description:
          "Error al procesar el consentimiento. Por favor, intenta de nuevo.",
        variant: "destructive",
      });
    } finally {
  ...
  const renderContent = () => (
    <div className={`rounded-lg border-2 p-6 ${getThemeColors()}`}>
      ...
      <div className="mb-6">
        ...
        {showFullDoc && (
          <div className="mt-3 p-4 bg-white rounded border text-sm text-gray-600">
            <p>
              📄 <strong>Documento:</strong> {docPath}
            </p>
            <p>
              🔗 <strong>Ubicación:</strong> GitHub Repository → {docPath}
            </p>
            <p>
              ⚖️ <strong>Evidencia Legal:</strong> IP, timestamp y hash serán
              registrados
            </p>
          </div>
        )}
      ...
      <div className="mt-4 text-xs text-gray-500">
        <p>
          🛡️ <strong>Protección Legal:</strong> Este consentimiento será
          registrado con tu IP ({userIP || "obteniendo..."}), timestamp y hash
          del contenido para evidencia legal según la normativa aplicable.
        </p>
      </div>
    </div>
  );
  ...
  if (variant === "banner") {
    return <div className="sticky top-0 z-40 shadow-lg">{renderContent()}</div>;
  }
