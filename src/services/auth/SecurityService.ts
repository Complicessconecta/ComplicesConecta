...
export class SecurityService {
  ...
  async analyzeUserActivity(
    ...
    try {
      ...
      const activityPatterns = await this.getUserActivityPatterns(
        userId,
        timeframe,
      );
      ...
      if (this.detectUnusualActivity(activityPatterns)) {
        flags.push({
          ...
          severity: "medium",
          description:
            "Patrón de actividad inusual detectado basado en historial",
          confidence: 0.8,
      ...
      if (Math.random() < 0.05) {
        // 5% chance de múltiples dispositivos
        flags.push({
  ...
  async detectFraud(
    ...
    try {
      ...
      const isHighVelocity = await this.checkActionVelocity(
        userId,
        activity.action,
      );
      if (isHighVelocity) {
      ...
      logger.error("Error detecting fraud:", {
        error: error instanceof Error ? error.message : String(error),
      });
      return {
        isFraudulent: false,
  ...
  async logSecurityEvent(
    ...
    try {
      ...
      const riskScore = await this.calculateEventRiskScore(
        action,
        auditDetails,
      );
      ...
      const { error } = await supabase.from("security").insert({
        user_id: userId,
        event_type: action,
  ...
  async getAuditLogs(
    ...
    try {
      ...
      if (error) {
        logger.error("Error getting audit logs from Supabase:", {
          error: error.message,
        });
        return { success: false, error: error.message };
      }
  ...
  private isSuspiciousIP(ip: string): boolean {
    ...
    const suspiciousIPs = [
      ...
      "10.0.0.0",
      "192.168.0.0",
      "172.16.0.0", // Rangos privados sospechosos
    ];
  ...
  private isUnusualUserAgent(userAgent: string): boolean {
    ...
    const suspiciousPatterns = [
      ...
      /wget/i,
      /python/i,
      /java/i, // Scripts automatizados
      /headless/i,
      /phantom/i,
      /selenium/i, // Headless browsers
      /postman/i,
      /insomnia/i,
      /httpie/i, // API testing tools
      /^$/, // User agent vacío
  ...
  private async checkActionVelocity(
    userId: string,
    action: string,
  ): Promise<boolean> {
    try {
      ...
      if (error) {
        logger.error("Error checking action velocity db:", {
          error: error.message,
        });
        return false;
      }
      ...
      logger.error("Error checking action velocity:", {
        error: error instanceof Error ? error.message : String(error),
      });
      return false;
    }
