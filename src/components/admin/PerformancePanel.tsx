...
export const PerformancePanel = () => {
  ...
  const loadPerformanceData = async () => {
    setIsLoading(true);
    try {
      await Promise.all([loadSystemMetrics(), loadRecentMetrics()]);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card
        className={`bg-white/10 backdrop-blur-md border border-white/20 ${health.status === "healthy" ? "border-green-400/30" : health.status === "warning" ? "border-yellow-400/30" : "border-red-400/30"}`}
      >
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              {health.status === "healthy" ? (
                ...
                <AlertTriangle
                  className={`w-8 h-8 ${health.status === "warning" ? "text-yellow-400" : "text-red-400"}`}
                />
              )}
              <div>
                ...
                <p className="text-sm text-white/80">
                  Uptime: {performanceData.uptime.toFixed(2)}% | Tasa de Error:{" "}
            ...
            <Card className="bg-white/10 backdrop-blur-md border border-white/20">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Badge
                    className={`${performanceData.errorRate > 3 ? "bg-red-500/20 text-red-300 border-red-400/30" : "bg-green-500/20 text-green-300 border-green-400/30"}`}
                  >
                    {performanceData.errorRate.toFixed(2)}%
                  </Badge>
                </div>
              ...
              {isLoading ? (
                ...
                <div className="space-y-3">
                  {recentMetrics.map((metric) => (
                    <div
                      key={metric.id}
                      className="flex items-center justify-between p-3 border border-white/20 rounded-lg bg-white/5 backdrop-blur-sm"
                    >
                      <div className="flex items-center gap-3">
              ...
              {metrics.length === 0 ? (
                ...
                <div className="space-y-2">
                  {metrics.map((metric) => (
                    <div
                      key={metric.id}
                      className="flex items-center justify-between p-2 hover:bg-white/10 rounded border border-white/10 bg-white/5 backdrop-blur-sm"
                    >
