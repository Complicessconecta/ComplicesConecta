...
export const DiscoverProfileCard = React.memo<DiscoverProfileCardProps>(
  ({ profile, onLike, onSuperLike }) => {
    const [imgSrc, setImgSrc] = useState<string>(
      profile.image_url ?? FALLBACK_IMAGE_URL,
    );
    ...
    const handleProfileClick = useCallback(() => {
      ...
      const profileType = profile.interested_in?.includes("couple")
        ? "couple"
        : "single";
          ...
          <div className="absolute bottom-0 left-0 p-2 sm:p-3 text-white w-full bg-gradient-to-t from-black/80 to-transparent">
            ...
            <div className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm overlay-text">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="truncate overlay-text">
                  {getLocationText()}
                </span>
              </div>
            </div>
