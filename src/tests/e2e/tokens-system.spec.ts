...
test.describe("Tokens - Comprar CMPX", () => {
  ...
  test("debe mostrar paquetes de tokens (1000 CMPX = $300 MXN)", async ({
    page,
  }) => {
    const packages = await page
      .locator('[data-testid="token-packages"], [class*="package"]')
      .count();
...
test.describe("Tokens - Balance y Dashboard", () => {
  ...
  test("debe mostrar balance de GTK", async ({ page }) => {
    const gtkBalance = await page
      .locator('[data-testid="gtk-balance"]')
      .first();
  ...
  test("debe mostrar historial de transacciones", async ({ page }) => {
    const history = await page
      .locator('[data-testid="transaction-history"]')
      .first();
...
test.describe("Tokens - Gastar CMPX", () => {
  ...
  test("debe mostrar error si saldo insuficiente", async ({ page }) => {
    const insufficientMessage = await page
      .locator("text=/insuficiente|no.*suficiente/i")
      .first();
...
test.describe("Tokens - Recibir como Creador", () => {
  ...
  test("debe poder retirar tokens ganados", async ({ page }) => {
    const withdrawButton = await page
      .locator('button:has-text("Retirar")')
      .first();
...
test.describe("Tokens - Notificaciones", () => {
  ...
  test("debe notificar cuando recibe tokens", async ({ page }) => {
    const receivedNotif = await page
      .locator('[data-testid="received-notification"]')
      .first();
...
test.describe("Tokens - Staking Básico", () => {
  ...
  test("debe mostrar opciones de staking", async ({ page }) => {
    const stakingOptions = await page
      .locator('[data-testid="staking-options"]')
      .first();
  ...
  test("debe mostrar tokens en staking", async ({ page }) => {
    const stakedTokens = await page
      .locator('[data-testid="staked-amount"]')
      .first();
...
test.describe("Tokens - Refunds", () => {
  ...
  test("debe poder solicitar reembolso si aplica", async ({ page }) => {
    const refundButton = await page
      .locator('button:has-text("Reembolso")')
      .first();
