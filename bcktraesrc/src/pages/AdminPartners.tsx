...
const AdminPartners = () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          country: clubForm.country || 'M├®xico',
  ...
  const handleCreateFlyer = async () => {
    ...
    try {
      ...
      toast({
        title: 'Flyer creado',
        description: 'El flyer ser├í procesado con watermark y blur autom├ítico',
      });

      // Procesar imagen autom├íticamente
      try {
        await processClubFlyerImageServer(flyerForm.image_url!, data.id);
        toast({
          title: 'Procesamiento iniciado',
          description: 'La imagen est├í siendo procesada con IA',
        });
      } catch (processError) {
        logger.error('Error procesando imagen:', { error: processError });
        toast({
          title: 'Advertencia',
          description: 'El flyer se cre├│ pero el procesamiento fall├│. Se reintentar├í autom├íticamente.',
          variant: 'destructive',
        });
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <h1 className="text-4xl font-bold mb-2">Gesti├│n de Partners (Clubs)</h1>
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          try {
                            ...
                            toast({
                              title: 'Procesamiento iniciado',
                              description: 'La imagen est├í siendo procesada',
                            });
                            loadFlyers();
                          } catch {
                            toast({
      ...
      <Dialog open={showClubDialog} onOpenChange={setShowClubDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Completa la informaci├│n del club</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
