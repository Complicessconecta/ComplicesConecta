...
export const PREMIUM_FEATURES: PremiumFeature[] = [
  ...
  },
  {
    ...
    description:
      "Sin límites en tus conexiones diarias, like a todos los perfiles",
    cost: 50,
    duration: 30,
    category: "discovery",
    icon: "❤️",
 ...
 */
export function hasUserPremiumAccess(
  userId: string,
  featureId: string,
): boolean {
...
export function purchasePremiumFeature(
  ...
  const expiresAt = new Date(
    now.getTime() + feature.duration * 24 * 60 * 60 * 1000,
  );
 ...
 */
export function getPremiumFeatureTimeLeft(
  userId: string,
  featureId: string,
): number {
  const userAccess = userPremiumAccess.get(userId) || [];
...
export function getPremiumStats(userId: string) {
  ...
  const totalSpent =
    userPremiumAccess
      .get(userId)
      ?.reduce((sum, access) => sum + access.cost, 0) || 0;
...
export function isPremiumEnabled(): boolean {
  ...
  return (
    import.meta.env.VITE_APP_PHASE === "beta" ||
