...
const testLogger = {
  ...
  warn: (message: string, data?: unknown) =>
    console.warn(`⚠️ [Integration.test] ${message}`, data || ""),
};
  ...
  describe("Integración entre Sistemas", () => {
    ...
    test("debe integrar ML matching con analytics", async () => {
      ...
      try {
        ...
        testLogger.info(
          "Integración analytics exitosa (ML matching deprecated)",
        );
      } catch (error) {
    ...
    test("debe integrar backup con cache", async () => {
      ...
      try {
        ...
        // Realizar backup
        const backupResult = await performManualBackup("incremental");
        testLogger.info("Backup realizado durante integración", {
          id: backupResult.id,
        });
  ...
  describe("Performance y Estabilidad", () => {
    test("debe manejar múltiples operaciones concurrentes", async () => {
      ...
      try {
        ...
        for (let i = 0; i < 5; i++) {
          promises.push(
            redisCache.set(`concurrent-${i}`, { value: i }, CacheTTL.SHORT),
          );
        }
