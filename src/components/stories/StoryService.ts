...
const getDemoStories = (): Story[] => {
  ...
  const userType = safeGetItem<string>("userType", {
    validate: false,
    defaultValue: "single",
  });
    ...
    try {
      const parsedUser =
        typeof demoUser === "string"
          ? JSON.parse(demoUser)
          : (demoUser as { first_name?: string; gender?: string } | null);
      if (
        parsedUser &&
        ...
        userAvatar = getRandomProfileImage(userGender, {
          width: 150,
          height: 150,
        });
      }
...
class StoryService {
  ...
  private getDemoStories(): Story[] {
    const stored = safeGetItem<Story[]>("demo_stories", {
      validate: false,
      defaultValue: null,
    });
    ...
    if (this.isDemoMode()) {
      ...
      if (story) {
        ...
        if (existingLike) {
          // Quitar like
          story.likes =
            story.likes?.filter((like) => like.userId !== "1") || [];
        } else {
          // Agregar like
