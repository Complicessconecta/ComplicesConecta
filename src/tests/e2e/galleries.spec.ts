...
test.describe("Galerías - Upload Foto Pública", () => {
  ...
  test("debe tener sección de galería en perfil", async ({ page }) => {
    const gallery = await page
      .locator('[data-testid="gallery"], [class*="gallery"]')
      .first();
  ...
  test("debe tener botón para subir foto", async ({ page }) => {
    const uploadButton = await page
      .locator('button:has-text("Subir"), input[type="file"]')
      .first();
...
test.describe("Galerías - Upload Foto Privada", () => {
  ...
  test("debe poder marcar foto como privada", async ({ page }) => {
    const privateToggle = await page
      .locator(
        'input[type="checkbox"][name*="private"], button:has-text("Privada")',
      )
      .first();
    expect(await privateToggle.count()).toBeGreaterThanOrEqual(0);
  });

  test("debe poder establecer precio en tokens para foto privada", async ({
    page,
  }) => {
  ...
  test("debe mostrar precio sugerido (90 CMPX)", async ({ page }) => {
    const suggestedPrice = await page
      .locator("text=/90.*CMPX|sugerido/i")
      .first();
...
test.describe("Galerías - Pagar para Ver", () => {
  ...
  test("debe mostrar precio al hacer click en foto privada", async ({
    page,
  }) => {
  ...
  test("debe confirmar antes de comprar acceso", async ({ page }) => {
    const buyButton = await page
      .locator('button:has-text("Comprar"), button:has-text("Desbloquear")')
      .first();
...
test.describe("Galerías - Comisiones", () => {
  ...
  test("debe mostrar información de comisión (90% creador)", async ({
    page,
  }) => {
...
test.describe("Galerías - Watermark IA", () => {
  ...
  test("debe aplicar watermark automáticamente", async ({ page }) => {
    const watermarkInfo = await page
      .locator("text=/watermark|marca de agua/i")
      .first();
    expect(await watermarkInfo.count()).toBeGreaterThanOrEqual(0);
  });

  test("debe tener opción de blur de caras/tatuajes", async ({ page }) => {
    const blurOption = await page
      .locator('input[name*="blur"], checkbox:has-text("Blur")')
      .first();
...
test.describe("Galerías - Validaciones", () => {
  ...
  test("debe validar cantidad máxima de fotos (50)", async ({ page }) => {
    const photoCount = await page
      .locator('[data-testid="photo-count"]')
      .first();
...
test.describe("Galerías - Eliminar Foto", () => {
  ...
  test("debe tener opción para eliminar foto", async ({ page }) => {
    const deleteButton = await page
      .locator('button:has-text("Eliminar"), button[data-action="delete"]')
      .first();
...
test.describe("Galerías - Ver Pública Gratis", () => {
  ...
  test("debe poder ver fotos públicas sin pagar", async ({ page }) => {
    const publicPhotos = await page
      .locator('[data-public="true"], img')
      .first();
...
test.describe("Galerías - Reportar Contenido", () => {
  ...
  test("debe tener opción para reportar foto", async ({ page }) => {
    const reportButton = await page
      .locator('button:has-text("Reportar")')
      .first();
...
test.describe("Galerías - Comentarios en Fotos", () => {
  ...
  test("debe poder comentar en foto", async ({ page }) => {
    const commentInput = await page
      .locator(
        'input[placeholder*="comentario"], textarea[placeholder*="comentario"]',
      )
      .first();
  ...
  test("debe mostrar lista de comentarios", async ({ page }) => {
    const comments = await page
      .locator('[data-testid="comments"], [class*="comments"]')
      .first();
