...
describe('TokenAnalyticsService', () => {
  ...
  describe('saveAnalytics', () => {
    it('should save analytics for a period', async () => {
      ...
      try {
        ...
        const result = await Promise.race([
          service.saveAnalytics('daily', startDate, endDate, mockMetrics),
          new Promise<Awaited<ReturnType<typeof service.saveAnalytics>>>((_, reject) => 
  ...
  describe('getHistoricalAnalytics', () => {
    it('should get historical analytics', async () => {
      ...
      try {
        const result = await Promise.race([
          service.getHistoricalAnalytics('daily', 7),
          new Promise<Awaited<ReturnType<typeof service.getHistoricalAnalytics>>>((_, reject) => 
            setTimeout(() => reject(new Error('Timeout')), maxTime)
          )
        ]).catch(() => {
  ...
  describe('generateAutomaticReport', () => {
    it('should generate automatic report', async () => {
      ...
      try {
        const result = await Promise.race([
          service.generateAutomaticReport('daily'),
          new Promise<Awaited<ReturnType<typeof service.generateAutomaticReport>>>((_, reject) => 
            setTimeout(() => reject(new Error('Timeout')), maxTime)
          )
        ]).catch(() => {
