  ...
  public: {
    Tables: {
      staking_records: {
        ...
        Insert: Omit<
          Database["public"]["Tables"]["staking_records"]["Row"],
          "id"
        >;
        Update: Partial<
          Database["public"]["Tables"]["staking_records"]["Insert"]
        >;
      };
    };
  };
...
describe("Supabase Integration Tests", () => {
    ...
    it("should validate staking data before insertion", async () => {
      ...
      const mockError = {
        ...
        error: {
          ...
          details:
            "Amount must be positive and duration must be at least 7 days",
        },
      };
  ...
  describe("Row Level Security (RLS)", () => {
    ...
    it("should allow access with proper authentication", async () => {
      ...
      expect(result.error).toBeNull();
      expect(result.data).toBeDefined();
      expect(result.data?.[0]?.user_id).toBe(
  ...
  describe("Real-time Subscriptions", () => {
    ...
    it("should handle subscription errors", async () => {
      ...
      expect(() => mockSubscriptionError.subscribe()).toThrow(
        "Subscription failed",
      );
    });
  });
