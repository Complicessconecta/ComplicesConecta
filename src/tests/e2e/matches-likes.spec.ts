...
test.describe("Matches - Ver Perfiles", () => {
  ...
  test("debe mostrar sección de discover/matches", async ({ page }) => {
    ...
    await page.waitForLoadState("networkidle");
    const discoverSection = await page
      .locator('[data-testid="discover"], main, [class*="discover"]')
      .first();
    expect(await discoverSection.count()).toBeGreaterThanOrEqual(0);
  });

  test("debe mostrar tarjetas de perfiles para hacer match", async ({
    page,
  }) => {
    await page.goto("/discover");
    await page.waitForLoadState("networkidle");
    const profileCards = await page
      .locator('[data-testid="profile-card"], [class*="card"], article')
      .count();
...
test.describe("Matches - Dar Like y Rechazar", () => {
  ...
  test("debe tener botón de like (corazón)", async ({ page }) => {
    const likeButton = await page
      .locator(
        'button[aria-label*="like"], button:has(svg[class*="heart"]), button[data-action="like"]',
      )
      .first();
  ...
  test("debe tener botón de rechazar (X)", async ({ page }) => {
    const rejectButton = await page
      .locator(
        'button[aria-label*="reject"], button[aria-label*="no"], button[data-action="reject"]',
      )
      .first();
...
test.describe("Matches - Super Like", () => {
  ...
  test("debe tener botón de super like (estrella)", async ({ page }) => {
    const superLikeButton = await page
      .locator(
        'button[aria-label*="super"], button:has(svg[class*="star"]), button[data-action="super-like"]',
      )
      .first();
    expect(await superLikeButton.count()).toBeGreaterThanOrEqual(0);
  });

  test("debe mostrar confirmación o animación al dar super like", async ({
    page,
  }) => {
    const superLikeButton = await page
      .locator('button[data-action="super-like"]')
      .first();
...
test.describe("Matches - Crear Match", () => {
  ...
  test("debe mostrar lista de matches existentes", async ({ page }) => {
    const matchesList = await page
      .locator('[data-testid="matches-list"], ul, [class*="matches"]')
      .first();
  ...
  test("debe mostrar notificación cuando hay nuevo match", async ({ page }) => {
    const notification = await page
      .locator('[role="alert"], [class*="notification"]')
      .first();
...
test.describe("Matches - Deshacer Match", () => {
  ...
  test("debe tener opción para deshacer match", async ({ page }) => {
    const unmatchButton = await page
      .locator('button:has-text("Deshacer"), button:has-text("Unmatch")')
      .first();
  ...
  test("debe confirmar antes de deshacer match", async ({ page }) => {
    const unmatchButton = await page
      .locator('button:has-text("Deshacer")')
      .first();
...
test.describe("Matches - Reportar y Bloquear", () => {
  ...
  test("debe tener opción para reportar perfil", async ({ page }) => {
    const reportButton = await page
      .locator('button:has-text("Reportar"), [data-action="report"]')
      .first();
  ...
  test("debe tener opción para bloquear perfil", async ({ page }) => {
    const blockButton = await page
      .locator('button:has-text("Bloquear"), [data-action="block"]')
      .first();
...
test.describe("Matches - Algoritmo ML y Filtros", () => {
  ...
  test("debe tener filtros de búsqueda (edad, distancia)", async ({ page }) => {
    const filtersButton = await page
      .locator('button:has-text("Filtros"), button[aria-label*="filter"]')
      .first();
    expect(await filtersButton.count()).toBeGreaterThanOrEqual(0);
  });

  test("debe mostrar score de compatibilidad si existe", async ({ page }) => {
    const compatibilityScore = await page
      .locator('[data-testid="compatibility"], [class*="score"]')
      .first();
...
test.describe("Matches - Límites Freemium", () => {
  ...
  test("debe mostrar contador de likes restantes si hay límite", async ({
    page,
  }) => {
    const likesCounter = await page
      .locator('[data-testid="likes-remaining"], [class*="counter"]')
      .first();
    expect(await likesCounter.count()).toBeGreaterThanOrEqual(0);
  });

  test("debe mostrar mensaje cuando se agotan likes diarios", async ({
    page,
  }) => {
    const limitMessage = await page
      .locator("text=/límite|limit|agotado/i")
      .first();
