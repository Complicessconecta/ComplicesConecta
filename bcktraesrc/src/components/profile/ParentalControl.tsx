...
export const ParentalControl = ({
  ...
  // Usar el hook centralizado para la l├│gica de PIN
  const {
    isLoading,
    verifyPin,
    setPin: setGlobalPin,
  ...
  const handlePinChange = async () => {
    const newPin = prompt("Nuevo PIN de 6 d├¡gitos (num├®rico):");
    if (newPin && newPin.length === 6 && /^\d+$/.test(newPin)) {
      ...
      toast.error("Entrada inv├ílida. El PIN debe ser de 6 d├¡gitos num├®ricos.");
    }
  };
  ...
  const getRestrictionDescription = (level: string) => {
    switch (level) {
      case "soft":
        return "ÔÜí Suave - Contenido sensible oculto, sin auto-bloqueo";
      case "medium":
        return "­ƒøí´©Å Moderado - Auto-bloqueo en 5 min de inactividad";
      case "strict":
        return "­ƒöÆ Estricto - M├íxima protecci├│n + Auto-bloqueo 5 min";
      default:
        return "ÔÜÖ´©Å Configuraci├│n personalizada";
    }
  };
          ...
          <Card className="w-full max-w-md bg-gradient-to-br from-purple-900/95 via-purple-800/95 to-blue-900/95 backdrop-blur-xl border border-white/20 shadow-2xl shadow-purple-900/50">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-6">
                {!hasPin ? (
                  <div className="bg-yellow-500/20 text-yellow-200 p-4 rounded-xl border border-yellow-400/30 text-center">
                    ...
                    <p className="text-sm">
                      Por favor, configura un PIN de 6 d├¡gitos en los ajustes de
                      tu perfil para usar esta funci├│n.
                    </p>
                  </div>
                ) : !showPinInput ? (
                  <Button
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex gap-3">
                      ...
                      >
                        {isLoading ? "Verificando..." : "Ô£à Confirmar"}
                      </Button>
                    </div>
                  </div>
                )}
              ...
              <div className="text-xs text-center text-white/60 border-t border-white/20 pt-4 mt-6">
                <p className="font-medium">­ƒöÆ Protecci├│n seg├║n Ley Olimpia</p>
  ...
  // Panel de configuraci├│n cuando est├í desbloqueado
  return (
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              {(["soft", "medium", "strict"] as const).map((level) => {
                ...
                const tooltipText =
                  level === "soft"
                    ? "Ligero: Solo aplica sobre la galer├¡a privada, sin bloqueo al iniciar sesi├│n."
                    : level === "strict"
                      ? "Estricto: El perfil puede iniciar bloqueado desde login y requiere PIN siempre."
                      : "Moderado: Auto-bloqueo en 5 minutos, recomendado para la mayor├¡a de usuarios.";
