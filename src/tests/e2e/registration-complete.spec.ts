...
test.describe("Registro Completo - Usuario Single", () => {
  ...
  test("debe validar todos los campos requeridos para Single", async ({
    page,
  }) => {
  ...
  test("debe validar campo de nombre (mínimo 2 caracteres)", async ({
    page,
  }) => {
    const nameInput = await page
      .getByLabel(/nombre/i)
      .or(page.locator('input[name*="name"], input[name*="firstName"]'))
      .first();
    ...
    if (await nameInput.isVisible().catch(() => false)) {
      ...
      const errorMessage = await page.getByText(
        /al menos 2 caracteres|mínimo 2|too short/i,
      );
  ...
  test("debe validar campo de edad (18-80 años)", async ({ page }) => {
    const ageInput = await page
      .getByLabel(/edad/i)
      .or(page.locator('input[name*="age"], input[type="number"]'))
      .first();
    ...
    if (await ageInput.isVisible().catch(() => false)) {
      ...
      const isInvalid = await ageInput
  ...
  test("debe validar campo de teléfono mexicano (10 dígitos)", async ({
    page,
  }) => {
  ...
  test("debe validar selección de género", async ({ page }) => {
    ...
    const genderSelect = await page
      .locator('select[name*="gender"], select[name*="género"]')
      .first();
  ...
  test("debe validar checkbox de términos y condiciones", async ({ page }) => {
    const termsCheckbox = await page
      .locator(
        'input[type="checkbox"][name*="terms"], input[type="checkbox"][name*="términos"]',
      )
      .first();
  ...
  test("debe mostrar enlace a términos y condiciones", async ({ page }) => {
    const termsLink = await page
      .locator('a[href*="terms"], a[href*="términos"], a[href*="legal"]')
      .first();
...
test.describe("Registro Completo - Usuario Pareja", () => {
  test.beforeEach(async ({ page }) => {
    ...
    const coupleButton = await page
      .locator('[data-testid="account-type-couple"], button:has-text("Pareja")')
      .first();
  ...
  test("debe mostrar campos adicionales para pareja", async ({ page }) => {
    ...
    const partnerNameInput = await page
      .getByLabel(/nombre.*pareja|partner.*name/i)
      .or(
        page.locator(
          'input[name*="partnerName"], input[name*="partner1"], input[name*="partner2"]',
        ),
      )
      .first();
    ...
    const coupleButton = await page
      .locator('button:has-text("Pareja"), [data-testid="account-type-couple"]')
      .first();
    if (await coupleButton.isVisible().catch(() => false)) {
      await coupleButton.click();
    ...
    const hasPartnerFields = await page
      .locator('input[name*="partner"], label:has-text("pareja")')
      .count();
  ...
  test("debe validar edad de ambos miembros de la pareja", async ({ page }) => {
    ...
    const ageInputs = await page
      .locator('input[name*="age"], input[type="number"]')
      .count();
  ...
  test("debe validar género de ambos miembros", async ({ page }) => {
    // Seleccionar pareja
    const coupleButton = await page
      .locator('button:has-text("Pareja")')
      .first();
    if (await coupleButton.isVisible().catch(() => false)) {
      await coupleButton.click();
      await page.waitForTimeout(1000);
    ...
    const genderSelects = await page
      .locator('select[name*="gender"], select[name*="género"]')
      .count();
...
test.describe("Validaciones de Contraseña", () => {
  ...
  test("debe validar longitud mínima de contraseña (6 caracteres)", async ({
    page,
  }) => {
    ...
    if (await passwordInput.isVisible()) {
      ...
      const errorMessage = await page.getByText(
        /mínimo|menos de|at least|minimum.*6/i,
      );
      const hasError = await errorMessage.isVisible().catch(() => false);
  ...
  test("debe aceptar contraseña válida", async ({ page }) => {
    ...
    if (await passwordInput.isVisible()) {
      ...
      const errorMessage = await page.getByText(
        /mínimo|menos de|at least|minimum/i,
      );
      await expect(errorMessage).not.toBeVisible();
    }
  });

  test("debe mostrar confirmación de contraseña si existe", async ({
    page,
  }) => {
    ...
    if (isVisible) {
      ...
      const errorMessage = await page.getByText(
        /no coinciden|don't match|must match/i,
      );
      const hasError = await errorMessage.isVisible().catch(() => false);
...
test.describe("Flujo Completo de Registro", () => {
  test("debe completar registro Single con TODOS los campos", async ({
    page,
  }) => {
    await page.goto("/auth");
    await page.waitForLoadState("networkidle");
    ...
    const phoneInput = await page
      ...
      .or(page.getByPlaceholder(/55 1234 5678/i))
      .first();
    if (await phoneInput.isVisible().catch(() => false)) {
      await phoneInput.fill("5512345678");
    }
  ...
  test("debe completar registro Pareja con TODOS los campos", async ({
    page,
  }) => {
    await page.goto("/auth");
    await page.waitForLoadState("networkidle");
