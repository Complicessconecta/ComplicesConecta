...
function validateClaimRequest(request: ClaimRequest): {
  valid: boolean;
  error?: string;
} {
  ...
  switch (request.rewardType) {
    case "world_id":
      if (!request.worldIdProof || typeof request.worldIdProof !== "object") {
        return {
          valid: false,
          error: "Prueba de World ID inválida o faltante",
        };
      }
      break;
    ...
    case "referral":
      if (
        ...
        typeof request.referralCode !== "string" ||
        request.referralCode.length < 6
      ) {
        return {
          valid: false,
          error: "Código de referido inválido (mínimo 6 caracteres)",
        };
      }
      break;
...
async function checkClaimLimits(
  ...
  if (error) {
    ...
    return {
      canClaim: false,
      error: "Error verificando historial de reclamaciones",
    };
  }
  ...
  if (previousClaims && previousClaims.length >= limits.maxClaims) {
    return {
      canClaim: false,
      error: `Máximo de reclamaciones alcanzado (${limits.maxClaims})`,
    };
  }
  ...
  if (limits.cooldown > 0 && previousClaims && previousClaims.length > 0) {
    ...
    if (timeDiff < limits.cooldown) {
      const remainingHours = Math.ceil(
        (limits.cooldown - timeDiff) / (60 * 60 * 1000),
      );
      return {
        canClaim: false,
        error: `Debes esperar ${remainingHours} horas para reclamar nuevamente`,
      };
    }
  }
...
async function checkMonthlyLimits(
  ...
  if (!userTokens) {
    return {
      canClaim: false,
      error: "Usuario no encontrado en sistema de tokens",
    };
  }
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(
      `🎁 Procesando recompensa tipo: ${rewardType} para usuario: ${user.id}`,
    );
    ...
    const claimLimitsCheck = await checkClaimLimits(
      supabaseClient,
      user.id,
      rewardType,
    );
    ...
    switch (rewardType) {
      case "world_id": {
        ...
        const { data: worldIdResult, error: worldIdError } =
          await supabaseClient.rpc("claim_world_id_reward", {
        ...
        if (worldIdError) {
          ...
          result = {
            success: false,
            message: "Error reclamando recompensa World ID",
          };
        } else {
      ...
      case "referral": {
        ...
        const { data: referralResult, error: referralError } =
          await supabaseClient.rpc("process_referral_reward", {
      ...
      case "beta_feedback": {
        if (!userTokens) {
          result = {
            success: false,
            message: "Error interno: datos de usuario no disponibles",
          };
          break;
        }
        ...
        if (feedbackError) {
          ...
          result = {
            success: false,
            message: "Error procesando recompensa de feedback",
          };
        } else {
          ...
          if (transactionError) {
            ...
            result = {
              ...
              remainingMonthly:
                userTokens.monthly_limit -
                (userTokens.monthly_earned + rewardAmount),
            };
      ...
      case "daily_login": {
        ...
        if (dailyError) {
          ...
          result = {
            success: false,
            message: "Error procesando recompensa diaria",
          };
        } else {
          ...
          if (transactionError) {
            console.error(
              "❌ Error registrando transacción diaria:",
              transactionError,
            );
            await supabaseClient
              .from("user_tokens")
            ...
            result = {
              success: false,
              message: "Error registrando transacción diaria",
            };
          } else {
            result = {
              success: true,
      ...
      case "profile_completion": {
        ...
        if (profileError) {
          ...
          result = {
            success: false,
            message: "Error procesando recompensa de perfil",
          };
        } else {
          const { error: transactionError } = await supabaseClient
            .from("transactions")
          ...
          if (transactionError) {
            console.error(
              "❌ Error registrando transacción de perfil:",
              transactionError,
            );
            await supabaseClient
              .from("user_tokens")
            ...
            result = {
              success: false,
              message: "Error registrando transacción de perfil",
            };
          } else {
            result = {
              success: true,
      ...
      default:
        result = {
          success: false,
          message: `Tipo de recompensa no soportado: ${rewardType}`,
        };
    }
