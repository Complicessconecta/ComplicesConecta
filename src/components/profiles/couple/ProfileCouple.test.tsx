...
vi.mock('@/components/ui/vanish-search-input', () => ({
  VanishSearchInput: () => <div data-testid="vanish-search-input">Search Input</div>
}));
...
vi.mock('@/components/ui/events-carousel', () => ({
  EventsCarousel: () => <div data-testid="events-carousel">Events Carousel</div>
}));
...
vi.mock('@/features/profile/coupleProfiles', () => ({
  ...
  useProfileScore: vi.fn(() => ({ score: 100, label: 'High', color: 'green', icon: '🏆' }))
}));

vi.mock('@/features/profile/useProfileScore', () => ({
  useProfileScore: () => ({ score: 95, label: 'Excelente', color: 'text-green-500', icon: '🏆' })
}));
