...
export const useAppPermissions = () => {
    ...
    for (const key in permissionConfig) {
      ...
      try {
        ...
        newStatus[type] =
          ...
          statusAny.receive ||
          "prompt";
        logger.info(`Permission status for ${type}:`, {
          status: newStatus[type],
        });
      } catch (error) {
        logger.error(`Error checking permission for ${type}:`, {
          error: String(error),
        });
  ...
  const requestPermission = useCallback(
    async (type: PermissionType): Promise<PermissionState> => {
      if (!Capacitor.isNativePlatform()) {
      ...
      try {
        ...
        logger.info(`Permission result for ${type}:`, {
          status: newStatusValue,
        });
        return newStatusValue;
      } catch (error) {
        logger.error(`Error requesting permission for ${type}:`, {
          error: String(error),
        });
  ...
  const checkAndRequestOnStartup = useCallback(async () => {
    ...
    for (const key in permissionConfig) {
      ...
      try {
        ...
        if (statusValue === "prompt") {
          logger.info(
            `Permissions: Permission for ${type} is 'prompt', requesting...`,
          );
        ...
        logger.error(`Error in startup check/request for ${type}:`, {
          error: String(error),
        });
    ...
    setIsLoading(false);
    logger.info("Permissions: Initial check and request complete.", {
