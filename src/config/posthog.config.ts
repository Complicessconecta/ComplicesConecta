...
export async function initPostHog(): Promise<void> {
  try {
          ...
          loaded: (_ph) => {
            ...
            if (supabase) {
              supabase.auth
                ...
                .catch((error) => {
                  logger.debug("Error obteniendo usuario para PostHog:", {
                    error:
    ...
    logger.error("Error inicializando PostHog", {
...
export function identifyUser(
  ...
  try {
    ...
    logger.error("Error identificando usuario en PostHog", {
      error: error instanceof Error ? error.message : String(error),
      userId: userId.substring(0, 8) + "***",
    });
  }
...
export function trackEvent(
  ...
  try {
    ...
    logger.error("Error registrando evento en PostHog", {
      error: error instanceof Error ? error.message : String(error),
      eventName,
    });
  }
...
export function resetPostHog(): void {
  try {
    ...
    logger.error("Error reseteando PostHog", {
      error: error instanceof Error ? error.message : String(error),
    });
  }
}
