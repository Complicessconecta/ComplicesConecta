...
export function calculateCompatibility(
  ...
  const totalUniqueInterests = new Set([...userInterests, ...profileInterests])
    .size;
...
export function generateMatchReasons(sharedInterests: string[]): string[] {
  ...
  if (
    sharedInterests.some((i) =>
      ["Hoteles Temáticos", "Cruceros Swinger", "Resorts Lifestyle"].includes(
        i,
      ),
    )
  ) {
  ...
  if (
    sharedInterests.some((i) =>
      ["Intercambio Suave", "Intercambio Completo"].includes(i),
    )
  ) {
...
export function filterAndSortByCompatibility(
  ...
  return profiles
    ...
    .filter(
      (profile) => profile.matchScore.compatibilityScore >= minCompatibility,
    )
    .sort(
      (a, b) =>
        b.matchScore.compatibilityScore - a.matchScore.compatibilityScore,
    );
}
