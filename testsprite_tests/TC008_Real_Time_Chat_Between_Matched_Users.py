...
async def run_test():
    ...
    try:
        ...
        # -> Close or skip the welcome introduction modal to access the main page and find the login option
        frame = context.pages[-1]
        # Click 'Saltar introducción' to skip the welcome introduction modal
        elem = frame.locator('xpath=html/body/div[2]/div/div/div[7]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click the 'Ingresar' button to open the login form for User A
        frame = context.pages[-1]
        ...
        # -> Input User A's email and password and click 'Iniciar Sesión' to login
        frame = context.pages[-1]
        # Input User A email
        ...
        frame = context.pages[-1]
        # Click 'Iniciar Sesión' button to login as User A
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Real-time message delivery confirmed').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: The test plan execution has failed because the real-time messaging between matched users did not work as expected.")
