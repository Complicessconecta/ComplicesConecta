...
test.describe("Critical User Flows", () => {
  ...
  test("Navigation Between Pages", async ({ page }) => {
    ...
    if (await page.locator(".md\\:hidden").isVisible()) {
      ...
      await expect(
        page.locator('[aria-label="Menú de navegación móvil"]'),
      ).toBeVisible();
    }
  });
  ...
  test("Profile Navigation and Functionality", async ({ page }) => {
    ...
    const profileTypeButtons = page.locator(
      '[data-testid="profile-type-toggle"]',
    );
  ...
  test("Chat Basic Functionality", async ({ page }) => {
    ...
    if (await firstChat.isVisible()) {
      ...
      await expect(
        page.locator('[placeholder="Escribe un mensaje..."]'),
      ).toBeVisible();
    }
  });
