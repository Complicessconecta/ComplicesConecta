  ...
  const requestPermission = async (
    ...
    try {
      ...
      if (type === "location") {
        ...
        setPermissions((prev) => ({
          ...prev,
          location: result.location as PermissionStatus,
        }));
      ...
      if (type === "notifications") {
        ...
        setPermissions((prev) => ({
          ...prev,
          notifications: result.receive as PermissionStatus,
        }));
...
export const PermissionManager: FC<PermissionManagerProps> = ({
  ...
  const currentPermission =
    pendingPermissions[currentStep] || pendingPermissions[0];
  ...
  const getIcon = (type: PermissionType) => {
    switch (type) {
      case "camera":
        return <CameraIcon className="h-12 w-12 text-blue-500" />;
      case "location":
        return <MapPin className="h-12 w-12 text-green-500" />;
      case "notifications":
        return <Bell className="h-12 w-12 text-yellow-500" />;
      default:
        return null;
    }
  };
  ...
  const getDescription = (type: PermissionType) => {
    switch (type) {
      case "camera":
        return "Necesaria para verificar tu perfil y subir fotos.";
      case "location":
        return "Para encontrar parejas cerca de ti.";
      case "notifications":
        return "Para saber cuándo tienes un nuevo match o mensaje.";
      default:
        return "";
    }
  };
    ...
    <div className="fixed inset-0 bg-white z-50 flex flex-col items-center justify-center p-6">
      <div className="max-w-md w-full text-center space-y-8">
        ...
        <div>
          ...
          <p className="text-gray-600">{getDescription(currentPermission!)}</p>
        </div>
