...
class SmartMatchingService {
  ...
  async getConversationStarters(
    ...
    return AdvancedFeaturesService.generateConversationStarters(
      userId,
      matchId,
    );
  }
  ...
  async findMatches(
    ...
    try {
      logger.info("🔍 Buscando matches para usuario", {
          ...
          filteredMatches.length > 0
            ? Math.round(
                filteredMatches.reduce(
                  (sum, m) => sum + m.totalScore + (m.socialScore || 0),
                  0,
                ) / filteredMatches.length,
              )
            : 0,
        highQualityMatches: filteredMatches.filter(
          (m) => m.totalScore + (m.socialScore || 0) >= 70,
        ).length,
      };
  ...
  async calculateCompatibility(
    ...
    try {
      ...
      if (!user1 || !user2) {
        logger.warn("Uno o ambos perfiles no encontrados", {
          userId1,
          userId2,
        });
        return null;
      ...
      logger.error("Error calculando compatibilidad:", {
        error: error instanceof Error ? error.message : String(error),
      });
      return null;
    }
  ...
  private async getCandidates(
    ...
    try {
      ...
      logger.error("Error obteniendo candidatos:", {
        error: error instanceof Error ? error.message : String(error),
      });
      return [];
    }
  ...
  > {
    ...
    try {
      ...
      const enrichedMatches = await Promise.all(
        matches.map(async (match) => {
          try {
            ...
            logger.warn(
              "Error enriqueciendo match con Neo4j, continuando sin enriquecimiento",
              {
                userId: userId.substring(0, 8) + "***",
                matchUserId: match.userId.substring(0, 8) + "***",
                error: error instanceof Error ? error.message : String(error),
      ...
      logger.error(
        "Error enriqueciendo matches con Neo4j, retornando matches originales",
        {
          error: error instanceof Error ? error.message : String(error),
        },
      );
  ...
  async getMatchesV2(
    ...
    try {
      ...
      if (isNeo4jEnabled && neo4jService) {
        try {
          // Obtener amigos mutuos desde Neo4j (requiere userId y targetUserId)
          // Como no tenemos un targetUserId específico, usamos un enfoque diferente
          const friendsOfFriends = await neo4jService.getFriendsOfFriends(
            userId,
            20, // limit
            true // excludeMatched
          );
          ...
          logger.warn(
            "⚠️ Error consultando Neo4j, continuando con Supabase solo",
            { error }
          );
        }
      }
      ...
      const stats = {
        totalCandidates: candidates.length,
        matchesFound: mergedResults.length,
        averageScore:
          mergedResults.length > 0
            ? mergedResults.reduce((sum, m) => sum + m.totalScore, 0) /
              mergedResults.length
            : 0,
        highQualityMatches: mergedResults.filter((m) => m.totalScore >= 70)
          .length,
      };

      logger.info("✅ Matches V2 encontrados con patrón Hydration", {
        userId: userId.substring(0, 8) + "***",
      ...
      return {
        matches: mergedResults.slice(0, options.limit || 10),
        total: mergedResults.length,
        stats,
      };
    } catch (error) {
      logger.error("❌ Error en getMatchesV2", {
        error: error instanceof Error ? error.message : String(error),
        userId: userId.substring(0, 8) + "***",
      });
      return this.emptyResult();
