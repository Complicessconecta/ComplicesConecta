...
export const useWorldID = () => {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      if (statsError && statsError.code !== "PGRST116") {
        logger.warn("Error fetching WorldID statistics:", {
  ...
  const isConfigured = useCallback(() => {
    return !!(
      import.meta.env.VITE_WORLD_APP_ID && import.meta.env.VITE_WORLD_APP_ACTION
    );
  }, []);
  ...
  const checkMonthlyLimit = useCallback(async () => {
    ...
    try {
      ...
      const current =
        data?.reduce((sum: number, reward: ReferralReward) => {
          const amount =
