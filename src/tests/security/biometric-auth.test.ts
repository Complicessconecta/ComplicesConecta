      ...
      error:
        (error as Error).name === "NotAllowedError"
          ...
          : (error as Error).name === "NotSupportedError"
            ...
            : (error as Error).name === "SecurityError"
              ? "Error de seguridad durante el registro"
              : "Error desconocido",
    };
  }
};
  ...
  describe("isWebAuthnSupported", () => {
    ...
    it("should return false when WebAuthn is not supported", () => {
      ...
      const credentialsSpy = vi
        .spyOn(global.navigator, "credentials", "get")
...
describe("BiometricSettings Component", () => {
  ...
  it("should show unsupported message when WebAuthn is not available", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("Autenticación Biométrica No Disponible"),
      ).toBeInTheDocument();
    });
  ...
  it("should show security information", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/datos biométricos nunca se almacenan/),
      ).toBeInTheDocument();
    });
  });
});
