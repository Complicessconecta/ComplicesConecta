...
const Requests = () => {
  ...
  const loadInvitations = useCallback(async () => {
    ...
    const { received, sent } =
      await invitationService.getInvitations(currentUserId);
  ...
  useEffect(() => {
    ...
    try {
      ...
      toast({
        title: "Advertencia",
        description:
          "Hubo un problema verificando la autenticación. Si persiste, intenta cerrar y abrir sesión.",
        variant: "destructive",
      });
    }
          ...
          prev.map((inv) =>
            inv.id === invitationId
              ? {
                  ...inv,
                  status:
                    action === "accept" ? "accepted" : ("declined" as const),
                }
              : inv,
          ),
        );
  ...
  const getStatusBadge = (status: Invitation["status"]) => {
    switch (status) {
      case "pending":
        return (
          <Badge className="text-yellow-400 border border-yellow-400 bg-transparent">
      ...
      case "declined":
        return (
          <Badge className="text-red-400 border border-red-400 bg-transparent">
      ...
      case "revoked":
        return (
          <Badge className="text-white border border-white/40 bg-transparent">
            <UserX className="h-3 w-3 mr-1" />
            Revocada
          </Badge>
        );
  ...
  const _pendingReceivedCount = receivedInvitations.filter(
    (inv) => inv.status === "pending",
  ).length;
  const _acceptedCount = [...receivedInvitations, ...sentInvitations].filter(
                    ...
                    receivedInvitations.map((inv) => (
                      ...
                      >
                        <div className="flex-1">
                          ...
                          {inv.message && (
                            <p className="text-sm text-white/70 bg-white/10 p-3 rounded-md mb-3">
                              "{inv.message}"
                            </p>
                          )}
                  ...
                  {sentInvitations.length === 0 ? (
                    ...
                    sentInvitations.map((inv) => (
                      ...
                      >
                        ...
                        {inv.message && (
                          <p className="text-sm text-white/70 bg-white/10 p-3 rounded-md my-3">
                            "{inv.message}"
                          </p>
                        )}
                        <p className="text-xs text-white/50">
