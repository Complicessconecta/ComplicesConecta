...
describe("UserIdentificationService", () => {
  ...
  describe("generateUniqueId", () => {
    it("should generate a unique ID correctly", async () => {
      ...
      const result = await userIdentificationService.generateUniqueId(
        "user-new",
        "single",
      );
      ...
      (supabase.from as any).mockReturnValue({
        select: vi.fn().mockReturnValue({
          eq: vi.fn().mockReturnValue({
            single: vi
              .fn()
              .mockResolvedValue({ data: existingId, error: null }),
          }),
        }),
        insert: insertMock,
      });
  ...
  describe("validateUniqueId", () => {
    it("should validate correct IDs", () => {
      expect(userIdentificationService.validateUniqueId("SNG-12345678")).toBe(
        true,
      );
      expect(userIdentificationService.validateUniqueId("CPL-00000001")).toBe(
        true,
      );
    });

    it("should reject incorrect IDs", () => {
      expect(userIdentificationService.validateUniqueId("ABC-12345678")).toBe(
        false,
      );
      expect(userIdentificationService.validateUniqueId("SNG-123")).toBe(false); // too short
      expect(userIdentificationService.validateUniqueId("SNG-123456789")).toBe(
        false,
      ); // too long
      expect(userIdentificationService.validateUniqueId("single-123")).toBe(
        false,
      );
    });
  });
