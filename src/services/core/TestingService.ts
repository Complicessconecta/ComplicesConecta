...
export class TestingService {
  ...
  async runTestSuite(
    ...
    const validTests = tests.filter(
      (t): t is () => Promise<void> => typeof t === "function",
    );
    let i = 0;
    ...
    const tests = [
      ...
      async () => {
        const { queryOptimizationService } =
          await import("@/services/core/QueryOptimizationService");
  ...
  async runIntegrationTests(): Promise<TestSuite> {
    const tests = [
      ...
      async () => {
        ...
        const cached = await advancedCacheService.get<{ test: boolean }>(
          "analytics_test",
        );
  ...
  async runE2ETests(): Promise<TestSuite> {
    const tests = [
      ...
      async () => {
        ...
        const cached = await advancedCacheService.get<{ success: boolean }>(
          "analytics_metrics",
        );
  ...
  async runAllTests(): Promise<{
    ...
    const summary = {
      totalTests:
        ...
        e2eTests.totalTests,
      passedTests:
        unitTests.passedTests +
        integrationTests.passedTests +
        e2eTests.passedTests,
      failedTests:
        unitTests.failedTests +
        integrationTests.failedTests +
        e2eTests.failedTests,
      totalCoverage:
        (unitTests.coverage + integrationTests.coverage + e2eTests.coverage) /
        3,
      totalDuration:
        unitTests.duration + integrationTests.duration + e2eTests.duration,
    };
  ...
  getTestStats(): {
    ...
    return {
      ...
      averageCoverage:
        suites.length > 0
          ? suites.reduce((sum, suite) => sum + suite.coverage, 0) /
            suites.length
          : 0,
      averageDuration:
        suites.length > 0
