...
export const FilterDemoCard: FC<FilterDemoCardProps> = ({
  card,
  index,
  onCtaClick,
}) => {
  ...
  const iconColor = useMemo(
    () => getFilterColor(card.filterType),
    [card.filterType],
  );
  ...
  const getHoverInfo = (filterType: string) => {
    switch (filterType) {
      case "age":
        return "Filtra perfiles por rango de edad para encontrar conexiones más compatibles";
      case "distance":
        return "Encuentra personas cerca de ti o amplía tu búsqueda según prefieras";
      case "interests":
        return "Conecta con personas que comparten tus gustos y estilo de vida";
      case "verified":
        return "Solo perfiles verificados para mayor seguridad y confianza";
      case "premium":
        return "Accede a usuarios premium con características exclusivas";
      case "online":
        return "Ve quién está conectado ahora para conversaciones inmediatas";
      default:
        return "Filtro personalizado para mejorar tu experiencia";
    }
  };
      ...
      >
        <Tooltip>
          <TooltipTrigger asChild>
            <Card
              className={`relative bg-white/10 backdrop-blur-md border border-white/20 hover:border-white/40 transition-all duration-300 group overflow-hidden h-full ${isHovered ? "bg-blue-500/20 shadow-lg shadow-blue-500/20" : ""}`}
            >
              <CardContent className="p-6 h-full flex flex-col relative z-10">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h3
                        className={`text-lg font-semibold text-white transition-all duration-300 ${isHovered ? "text-blue-200" : ""}`}
                      >
                        {card.title}
                      </h3>
                      <Badge
                ...
                {/* Description */}
                <p
                  className={`text-white/80 text-sm mb-4 leading-relaxed transition-all duration-300 ${isHovered ? "text-blue-100" : ""}`}
                >
                  {card.description}
                </p>
                ...
                {/* Demo Value */}
                <div
                  className={`mb-4 p-3 bg-white/5 rounded-lg border border-white/10 transition-all duration-300 ${isHovered ? "bg-blue-500/10 border-blue-400/30" : ""}`}
                >
                  <div className="flex items-center justify-between">
                    <span
                      className={`text-white/60 text-xs transition-all duration-300 ${isHovered ? "text-blue-200" : ""}`}
                    >
                      Valor de ejemplo:
                    </span>
                    <Badge
                ...
                {/* Explanation */}
                <p
                  className={`text-white/70 text-xs mb-4 italic transition-all duration-300 ${isHovered ? "text-blue-200" : ""}`}
                >
                  {card.explanation}
                </p>
                ...
                <div className="mt-3 text-center">
                  <span
                    className={`text-xs text-white/50 transition-all duration-300 ${isHovered ? "text-blue-300" : ""}`}
                  >
          ...
          <TooltipContent
            side="top"
            className="bg-blue-900/90 border-blue-500/50 text-white max-w-xs"
          >
