...
async def run_test():
    ...
    try:
        ...
        # -> Click on 'Ingresar' button to navigate to the login page
        ...
        # -> Click on 'Saltar introducción' button with index 76 to dismiss the welcome modal and access login form
        frame = context.pages[-1]
        # Click 'Saltar introducción' button to dismiss welcome modal
        elem = frame.locator('xpath=html/body/div[2]/div/div/div[7]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click on 'Ingresar' button with index 11 to navigate to login page
        frame = context.pages[-1]
        # Click on 'Ingresar' button to go to login page
        ...
        # -> Input valid email and password for user with MFA enabled and submit login form
        frame = context.pages[-1]
        # Enter valid email for user with MFA enabled
        ...
        await page.wait_for_timeout(3000); await elem.fill('valid_mfa_user@example.com')
        ...
        frame = context.pages[-1]
        # Enter valid password for user with MFA enabled
        elem = frame.locator('xpath=html/body/div/div/div/div/div/main/div/div/div/div[2]/div/div[2]/div/div[2]/div/div[2]/form/div[2]/input').nth(0)
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=MFA verification successful').first).to_be_visible(timeout=30000)
        except AssertionError:
            raise AssertionError("Test failed: MFA challenge was not presented or validated as expected during login flow with MFA enabled.")
