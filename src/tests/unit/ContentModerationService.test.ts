...
describe('ContentModerationService', () => {
  ...
  describe('moderateText', () => {
    it('should approve clean text', async () => {
      const result = await contentModerationService.moderateText('Hola, ¿cómo estás?');
    ...
    it('should detect toxic content', async () => {
      ...
      const resultAggressive = await contentModerationService.moderateText(aggressiveText);

      expect(resultAggressive.flags.some(f => f.type === 'harassment')).toBe(true);
  ...
  describe('moderateProfile', () => {
    it('should flag incomplete profiles', async () => {
      ...
      const result = await contentModerationService.moderateProfile(incompleteProfile);
    ...
    it('should flag suspicious names', async () => {
      ...
      const result = await contentModerationService.moderateProfile(suspiciousProfile);
      
      expect(result.flags.some(f => f.type === 'fake_profile' && f.description.includes('Nombre'))).toBe(true);
    });
    ...
    it('should flag underage profiles', async () => {
      ...
      const result = await contentModerationService.moderateProfile(underageProfile);
  ...
  describe('logModerationResult', () => {
    it('should log to supabase', async () => {
      ...
      await contentModerationService.logModerationResult('text', 'content-123', result, 'user-123');
    ...
    it('should handle supabase errors gracefully', async () => {
      ...
      await contentModerationService.logModerationResult('text', 'content-123', result);
