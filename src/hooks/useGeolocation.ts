...
export const useGeolocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        try {
          ...
          logger.warn("Error calculating S2 cell ID:", {
      ...
      return users
        .filter((user) => {
          ...
          if (
            ...
            user.location.accuracy > filter.minAccuracy
          ) {
            return false;
          }
  ...
  const isLocationAccurate = useCallback(
    (minAccuracy: number = 100): boolean => {
  ...
  const getPermissionStatus =
    useCallback(async (): Promise<PermissionState | null> => {
      ...
      try {
        ...
        logger.warn("Could not query geolocation permission:", {
          error: error instanceof Error ? error.message : String(error),
        });
        return null;
      }
