...
export const EnhancedGallery: React.FC<GalleryProps> = ({
  ...
  const checkModeAndLoadImages = async () => {
    ...
    if (isDemo) {
      ...
      setLoading(false);
      logger.info("🎭 Galería demo cargada:", {
        profileType,
        imageCount: getDemoImages(profileType).length,
      });
    } else {
  ...
  const loadRealImages = async () => {
    try {
      ...
      if (publicError) {
        logger.error("Error cargando imágenes públicas:", {
          error: String(publicError),
        });
      }
      ...
      const allImages = [...(publicImages || []), ...(privateImages || [])].map(
        (img: unknown) => {
      ...
      setImages(allImages);
      logger.info("✅ Galería real cargada:", {
        userId,
  ...
  const _handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      const newImage: GalleryImage = {
        ...
        url:
          (imageData as { file_url?: string; thumbnail_url?: string })
            .file_url ||
          (imageData as { file_url?: string; thumbnail_url?: string })
            .thumbnail_url ||
          "/placeholder.svg",
        caption: "",
        isPublic:
          (imageData as { is_public?: boolean | null }).is_public ?? false,
        uploadedAt:
          (imageData as { created_at?: string | null }).created_at ||
    ...
    try {
      ...
      if (image && supabase && image.url && !image.url.startsWith("/")) {
        try {
          ...
          if (storageError) {
            logger.error("Error eliminando de storage:", {
              error: String(storageError),
            });
          }
        } catch (storageErr) {
          logger.warn("No se pudo eliminar de storage:", {
    ...
    <Card className="bg-card/80 backdrop-blur-sm border border-primary/10">
      <CardHeader>
        <CardTitle className="text-white flex items-center justify-between">
          ...
          {isDemoMode && (
            <Badge className="bg-purple-500/80 text-white">🎭 Modo Demo</Badge>
          )}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="public" className="w-full">
          ...
          <TabsContent value="public" className="mt-6">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {(showAll
                ? publicImages
                : publicImages.slice(0, imagesPerPage)
              ).map((image) => (
                  ...
                  {isOwner && (
                    <div className="absolute top-2 right-2 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          toggleImageVisibility(image.id, image.isPublic)
                        }
                      >
