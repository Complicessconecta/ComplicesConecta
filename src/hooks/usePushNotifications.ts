...
export const usePushNotifications = ({
  ...
  const saveSubscriptionToDatabase = useCallback(
    async (pushSubscription: PushSubscription) => {
      ...
      try {
        ...
        logger.error("Error saving subscription to database:", {
  ...
  const removeSubscriptionFromDatabase = useCallback(
    async (_pushSubscription: PushSubscription) => {
      if (!userId) return;
      ...
      try {
        ...
        logger.error("Error removing subscription from database:", {
          error: error instanceof Error ? error.message : String(error),
        });
        throw error;
      }
  ...
  const showLocalNotification = useCallback(
    (payload: PushNotificationPayload) => {
      ...
      try {
        ...
        logger.error("Error showing local notification:", {
          error: error instanceof Error ? error.message : String(error),
        });
        return null;
      }
  ...
  useEffect(() => {
    const loadExistingSubscription = async () => {
      ...
      try {
        ...
        logger.error("❌ Error cargando suscripción existente:", {
          error: error instanceof Error ? error.message : String(error),
        });
      }
    };
