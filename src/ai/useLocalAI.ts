...
export interface UseLocalAIResult {
  ...
  sendMessage: (
    text: string,
    runtimeStateOverride?: Partial<LegalRuntimeState>,
  ) => Promise<void>;
}
...
export function useLocalAI(options?: UseLocalAIOptions): UseLocalAIResult {
  ...
  const runtimeStateRef = useRef<LegalRuntimeState | undefined>(
    options?.initialRuntimeState,
  );
  ...
  const sendMessage = useCallback<UseLocalAIResult["sendMessage"]>(
    async (text, runtimeStateOverride) => {
