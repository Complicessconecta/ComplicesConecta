...
export function optimizeImageUrl(
  ...
  try {
    ...
    if (typeof widthValue === "number")
      u.searchParams.set("w", String(Math.round(widthValue)));
    if (typeof heightValue === "number")
      u.searchParams.set("h", String(Math.round(heightValue)));
...
export function generateSrcSet(
  ...
  return widths
    ...
    .map(
      (w) =>
        `${optimizeImageUrl(url, { ...options, width: w })} ${Math.round(w)}w`,
    )
    .join(", ");
}
