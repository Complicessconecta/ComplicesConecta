...
describe("Profile Cache Tests", () => {
  ...
  describe("useProfile Hook", () => {
    ...
    it("debe manejar errores de carga correctamente", async () => {
      ...
      try {
        ...
        const { result } = renderHook(() => useProfile("nonexistent-id"), {
          wrapper: createWrapper(),
        });
        ...
        if (elapsed >= maxTime) {
          console.warn(
            "⚠️ [Profile Cache Test] Timeout alcanzado, saliendo del test",
          );
    ...
    it("debe usar cache correctamente", async () => {
      ...
      // Primera llamada
      const { result: result1 } = renderHook(() => useProfile("test-user-id"), {
        wrapper,
      });
      ...
      const { result: result2 } = renderHook(() => useProfile("test-user-id"), {
        wrapper,
      });
  ...
  describe("Cache Management", () => {
    ...
    it("debe generar keys de cache correctamente", () => {
      ...
      expect(profileKeys.list("filter")).toEqual([
        "profiles",
        "list",
        { filters: "filter" },
      ]);
      expect(profileKeys.details()).toEqual(["profiles", "detail"]);
      expect(profileKeys.detail("user-id")).toEqual([
        "profiles",
        "detail",
        "user-id",
      ]);
