...
export class AdvancedFeaturesService {
  ...
  static async generateAdvancedMatches(
    ...
    try {
      ...
      logger.error("Error generating advanced matches:", {
  ...
  private static async calculateAdvancedCompatibility(
    ...
    if (interestScore > 0.7) {
      reasons.push(
        `Comparten ${Math.round(interestScore * 100)}% de preferencias lifestyle`,
      );
    }
  ...
  private static calculateGenderCompatibility(
    ...
    const user1InterestedInUser2 =
      interestedIn1.toLowerCase().includes(gender2.toLowerCase()) ||
      interestedIn1.toLowerCase() === "all";
    const user2InterestedInUser1 =
      interestedIn2.toLowerCase().includes(gender1.toLowerCase()) ||
      interestedIn2.toLowerCase() === "all";
  ...
  private static calculatePersonalityCompatibility(
    ...
    const bioWords1 = bio1
      .toLowerCase()
      .split(/\s+/)
      .filter((word: string) => word.length > 3);
    const bioWords2 = bio2
      .toLowerCase()
      .split(/\s+/)
      .filter((word: string) => word.length > 3);
    const commonBioWords = bioWords1.filter((word: string) =>
      bioWords2.includes(word),
    );

    if (bioWords1.length > 0 && bioWords2.length > 0) {
      compatibility +=
        (commonBioWords.length / Math.max(bioWords1.length, bioWords2.length)) *
        0.3;
    }
    ...
    for (const [_trait, keywords] of Object.entries(personalityKeywords)) {
      ...
      const user2HasTrait = keywords.some(
        (keyword) =>
          ...
          interests2.some((interest: string) =>
            interest.toLowerCase().includes(keyword),
          ),
      );
  ...
  private static calculateConfidenceLevel(
    ...
    for (const field of requiredFields) {
      ...
      if (field === "age") {
        ...
        if (value1 && typeof value1 === "string" && value1.trim().length > 0)
          user1Completeness++;
    ...
    return (
      (user1Completeness + user2Completeness) / (requiredFields.length * 2)
    );
  }
  ...
  private static async storeRecommendations(
    ...
    try {
      ...
      logger.error("Error calculating compatibility:", {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
   ...
   */
  static async generatePersonalityInsights(
    userId: string,
    ...
    try {
      ...
      const personalityAnalysis = this.analyzePersonalityFromBio(
        bio,
        interests,
      );
      ...
      logger.error("Error generating personality insights:", {
        error: error instanceof Error ? error.message : String(error),
      });
      return [];
    }
   ...
   */
  private static analyzePersonalityFromBio(
    bio: string,
    interests: string[],
  ): Record<string, { score: number; description: string; factors: string[] }> {
    ...
    const opennessScore = this.calculateTraitScore(
      bio,
      interests,
      opennessKeywords,
    );
    ...
    const conscientiousnessScore = this.calculateTraitScore(
      bio,
      interests,
      conscientiousnessKeywords,
    );
    ...
    const extraversionScore = this.calculateTraitScore(
      bio,
      interests,
      extraversionKeywords,
    );
    ...
    const agreeablenessScore = this.calculateTraitScore(
      bio,
      interests,
      agreeablenessKeywords,
    );
    ...
    const neuroticismScore =
      100 - this.calculateTraitScore(bio, interests, neuroticismKeywords); // Invertido
    ...
    const lifestyleScore = this.calculateTraitScore(
      bio,
      interests,
      lifestyleKeywords,
    );
    ...
    const communicationScore = this.calculateTraitScore(
      bio,
      interests,
    ...
    const discretionScore = this.calculateTraitScore(
      bio,
      interests,
      discretionKeywords,
    );
  ...
  private static getLifestyleOpennessDescription(score: number): string {
    if (score >= 80)
      return "Muy abierto a nuevas experiencias y exploración lifestyle";
    if (score >= 60)
  ...
  private static getCommunicationSkillsDescription(score: number): string {
    if (score >= 80)
      return "Excelente comunicación, respeta límites y consenso mutuo";
    ...
    if (score >= 20)
      return "Comunicación básica, necesita trabajar en consenso";
  ...
  private static getOpennessDescription(score: number): string {
    if (score >= 80)
      return "Muy abierto a nuevas experiencias, creativo y aventurero";
  ...
  static async generateConversationStarters(
    ...
    try {
      ...
      const commonInterests = (userProfile.interests || []).filter(
        (interest: string) => (matchProfile.interests || []).includes(interest),
      );
      ...
      if (userInsights.length > 0 && matchInsights.length > 0) {
        ...
        const complementaryStarters = this.generateComplementaryStarters(
          userInsights,
          matchInsights,
        );
    ...
    const complementaryPairs = [
      {
        user: "extraversion",
        match: "introversion",
        text: "Me gusta conocer gente nueva, pero también valoro la intimidad. ¿Cómo equilibra tú la socialización?",
      },
      {
        user: "openness",
        match: "conscientiousness",
        text: "Soy bastante espontáneo, pero también me gusta tener planes. ¿Cómo manejas tú el equilibrio entre espontaneidad y organización?",
      },
      {
        user: "agreeableness",
        match: "assertiveness",
        text: "Suelo ser muy comprensivo, pero a veces necesito ser más directo. ¿Cómo encuentras tú el equilibrio entre amabilidad y asertividad?",
      },
    ];
  ...
  static async scheduleVirtualDate(
    ...
    try {
      const virtualDate: VirtualDate = {
        ...
        duration_minutes: activities.reduce(
      ...
      logger.error("Error scheduling virtual date:", {
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
    }
  ...
  static getVirtualDateActivities(): VirtualActivity[] {
    return [
      ...
      },
      {
        ...
        config: {
          romantic_settings: ["beach_sunset", "cozy_cabin", "luxury_suite"],
      ...
      },
      {
        ...
        type: "video_call",
        config: {
          topics: ["boundaries", "preferences", "experiences"],
          safe_space: true,
        },
        duration_minutes: 45,
      },
      {
        ...
        type: "group_session",
        config: {
          workshop_types: ["communication", "exploration", "connection"],
      ...
      },
      {
        ...
        type: "educational",
        config: {
          topics: ["boundaries", "communication", "respect", "safety"],
      ...
      },
      {
        ...
        type: "assessment",
        config: {
          test_types: ["lifestyle", "preferences", "boundaries"],
      ...
      },
      {
        ...
        type: "planning",
        config: {
          party_types: ["intimate", "group", "themed"],
      ...
      },
      {
        ...
        type: "storytelling",
        config: {
          story_types: ["experiences", "adventures", "learnings"],
          anonymous_mode: true,
        },
        duration_minutes: 90,
      },
  ...
  static async getComprehensiveCompatibilityAnalysis(
    ...
    try {
      ...
      const personalityCompatibility =
        this.calculatePersonalityCompatibilityFromInsights(
          userInsights,
          matchInsights,
        );
      ...
      const analysis = {
        ...
        breakdown: {
          ...
          lifestyle: compatibility.score * 0.2,
          personality: personalityCompatibility * 0.15,
          education: compatibility.score * 0.1,
        },
      ...
      logger.error("Error in comprehensive compatibility analysis:", {
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
    }
  ...
  private static calculatePersonalityCompatibilityFromInsights(
    ...
    for (const userTrait of userInsights) {
      ...
      if (matchTrait) {
        ...
        const similarity =
          1 - Math.abs(userTrait.score - matchTrait.score) / 100;
  ...
  private static identifyCompatibilityChallenges(
    ...
    const ageDiff = Math.abs(
      (userProfile.age || 25) - (matchProfile.age || 25),
    );
    if (ageDiff > 10) {
    ...
    if (
      ...
      const distance =
        Math.sqrt(Math.pow(lat2 - lat1, 2) + Math.pow(lon2 - lon1, 2)) * 111; // km aproximados
  ...
  private static generateCompatibilityRecommendations(
    ...
    const userExtraversion = userInsights.find(
      (i) => i.trait === "extraversion",
    );
    const matchExtraversion = matchInsights.find(
      (i) => i.trait === "extraversion",
    );
    ...
    if (
      ...
      if (distance < 20) {
        recommendations.push(
          "Aprovecha su cercanía para encuentros presenciales discretos",
        );
      } else if (distance < 100) {
        recommendations.push(
          "Considera planear encuentros con anticipación dada la distancia",
        );
      } else {
        recommendations.push(
          "Considera citas virtuales hasta que puedan encontrarse",
        );
      }
    } else {
  ...
  static async updatePreferencesFromBehavior(
    ...
    try {
      ...
      const preferenceUpdates = this.analyzePreferencePatterns(
        (likedProfiles || []) as unknown as ProfileRow[],
        (passedProfiles || []) as unknown as ProfileRow[],
      );
      ...
      logger.info("User preferences updated based on behavior:", {
        userId,
        updates: preferenceUpdates,
      });
    } catch (error) {
      logger.error("Error updating learning preferences:", {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
   ...
   */
  private static analyzePreferencePatterns(
    likedProfiles: ProfileRow[],
    _passedProfiles: ProfileRow[],
