...
DO $$
BEGIN
    -- Asegurar que la columna death_clause_text exista aunque la tabla couple_agreements
    -- se haya creado previamente sin esta columna
    IF EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public'
          AND table_name = 'couple_agreements'
    ) THEN
        -- Asegurar cláusula de disposición de activos
        ALTER TABLE couple_agreements
            ADD COLUMN IF NOT EXISTS asset_disposition_clause TEXT NOT NULL DEFAULT 'ADMIN_FORFEIT';

        -- Asegurar texto de cláusula de muerte súbita
        ALTER TABLE couple_agreements
            ADD COLUMN IF NOT EXISTS death_clause_text TEXT NOT NULL DEFAULT 
                'En caso de disolución de la cuenta de pareja por conflicto no resuelto en 30 días, los activos digitales (Tokens/NFTs) no reclamados serán transferidos a la plataforma por concepto de "Gastos Administrativos de Cancelación" y la cuenta será eliminada.';
    END IF;
END $$;
...
DO $$
BEGIN
    -- Crear índice solo si existe la columna resolution_type en couple_disputes
    IF EXISTS (
    ...
    ELSE
        RAISE NOTICE '⚠️ Columna resolution_type no existe en couple_disputes; se omite índice idx_couple_disputes_deadline.';
    END IF;
END $$;
...
DROP TRIGGER IF EXISTS trigger_complete_couple_agreement ON couple_agreements;
...
DROP TRIGGER IF EXISTS trigger_auto_forfeit_disputes ON couple_agreements;
...
DROP POLICY IF EXISTS "Partners can view own agreements" ON couple_agreements;
...
DROP POLICY IF EXISTS "Partners can update own agreements" ON couple_agreements;
...
DO $$
BEGIN
    -- Crear política solo si existe la columna couple_agreement_id en couple_disputes
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public'
          AND table_name = 'couple_disputes'
        ...
        IF EXISTS (
            ...
            WHERE schemaname = 'public'
              ...
              AND policyname = 'Partners can view own disputes'
        ) THEN
    ...
    ELSE
        RAISE NOTICE '⚠️ Columna couple_agreement_id no existe en couple_disputes; se omite política Partners can view own disputes.';
    END IF;
END $$;
