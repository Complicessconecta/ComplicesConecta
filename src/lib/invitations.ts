...
export const invitationService = {
  async sendInvitation(
    ...
    try {
      ...
      if (error) {
        logger.error("❌ Error enviando invitación:", {
      ...
      logger.error("❌ Error en sendInvitation:", {
  ...
  async acceptInvitation(invitationId: string): Promise<void> {
    try {
      ...
      if (error) {
        logger.error("❌ Error actualizando invitación:", {
          error: error.message || String(error),
        });
        throw error;
      }
    } catch (error) {
      logger.error("❌ Error en acceptInvitation:", {
        error: error instanceof Error ? error.message : String(error),
      });
      // Fallback a mock data
      const invitation = mockInvitations.find((inv) => inv.id === invitationId);
  ...
  async declineInvitation(invitationId: string): Promise<void> {
    try {
      ...
      logger.error("❌ Error en declineInvitation:", {
        error: error instanceof Error ? error.message : String(error),
      });
      // Fallback a mock data
      const invitation = mockInvitations.find((inv) => inv.id === invitationId);
  ...
  async getInvitations(
    profileId: string,
  ): Promise<{ received: Invitation[]; sent: Invitation[] }> {
    try {
      if (!supabase) {
        logger.error("Supabase no está disponible");
        throw new Error("Supabase no está disponible");
      ...
      if (receivedError || sentError) {
        logger.error("❌ Error obteniendo invitaciones:", {
          error:
            (receivedError || sentError)?.message ||
            String(receivedError || sentError),
        });
      ...
      logger.error("❌ Error en getInvitations:", {
        error: error instanceof Error ? error.message : String(error),
      });
      // Fallback a mock data
      const received = mockInvitations.filter(
        (inv) => inv.to_profile === profileId,
      );
      const sent = mockInvitations.filter(
        (inv) => inv.from_profile === profileId,
      );
      return { received, sent };
    }
  },
  ...
  async hasGalleryAccess(owner: string, grantee: string): Promise<boolean> {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select("id")
        .or(
          `and(owner_profile_id.eq.${owner},grantee_profile_id.eq.${grantee}),and(profile_id.eq.${owner},granted_to.eq.${grantee})`,
        )
        .eq("status", "active")
        .limit(1);
      ...
      logger.error("❌ Error en hasGalleryAccess:", {
        error: error instanceof Error ? error.message : String(error),
      });
      // Fallback a mock data
      return mockGalleryPermissions.some(
        (perm) =>
          perm.owner_profile === owner &&
  ...
  async revokeGalleryAccess(owner: string, grantee: string): Promise<void> {
    try {
      ...
      if (error) {
        logger.error("❌ Error otorgando permiso de galería:", {
          error: error instanceof Error ? error.message : String(error),
        });
        throw error;
      }
    } catch (error) {
      logger.error("❌ Error en revokeGalleryAccess:", {
        error: error instanceof Error ? error.message : String(error),
      });
      // Fallback a mock data
  ...
  async hasChatAccess(user1: string, user2: string): Promise<boolean> {
    try {
      ...
      if (!isValidUUID(user1) || !isValidUUID(user2)) {
        logger.warn("⚠️ UUIDs inválidos para hasChatAccess, usando fallback:", {
          user1,
          user2,
        });
        ...
        return mockInvitations.some(
          (inv) =>
            ((inv.from_profile === user1 && inv.to_profile === user2) ||
      ...
      const { data, error } = await supabase
        ...
        .select("id")
        .or(
          `and(from_profile.eq.${user1},to_profile.eq.${user2}),and(from_profile.eq.${user2},to_profile.eq.${user1})`,
        )
        .eq("type", "chat")
        .eq("status", "accepted")
        .limit(1);
      ...
      logger.error("❌ Error en hasChatAccess:", {
        error: error instanceof Error ? error.message : String(error),
      });
      // Fallback a mock data
      return mockInvitations.some(
